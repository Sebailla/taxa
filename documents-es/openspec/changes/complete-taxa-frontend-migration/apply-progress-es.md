...
|--------|---------|----------------------------|-----------------|--------|
| PR 3a | **Bootstrap de toolchain** (NUEVA posición 1) | ~210 | `package.json` (modificado, `next@^16` / `react@^19` / `react-dom@^19` / `tailwindcss@^4` / toolchain TS / `engines.node ">=20.9.0"` / `scripts.check-runtime` / `scripts.build:web`; deps legacy de Tailwind 3.4 eliminadas) + `scripts/check-runtime.mjs` (nuevo, Node ≥ 20.9.0) + `tsconfig.json` (nuevo en la raíz del repo, config base + aliases de ruta `@taxa/<capability>`) + `.nvmrc` (nuevo, pin `20`) + `tests/test_toolchain_bootstrap.py` (nuevo) + `tests/test_check_runtime.py` (nuevo) | pendiente de reconstrucción |
...
el presupuesto de 400 líneas con -40 LoC de holgura; **no
se requiere `size:exception`**). El más pesado de los
