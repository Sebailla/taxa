...
- **5b.3 — presentation de `FileExplorer` + `FileViewer` + comportamiento
  ...
  **debounce de 200 ms**, modos filter / highlight, y
  ...
  selectores `@layer components` de PR 3c-c. Sin `SearchTab` /
        ...
        - **Alcance (esta entrada)**. PR 5c.1b-B es la rebanada React + CSS
          ...
          - **Diferimientos (vinculantes, esta entrada)**:
            ...
            - **Defecto 2 — drift de `top` de header / tabs
              ...
              aliases 3c-e1 / 3c-e2 existentes de `--primary-fixed` /
              `--on-primary-fixed` / `--surface-container-lowest`) y
...
- **Alcance de este intento (vinculante)**: superficies de
    ...
    - **Alcance (esta entrada)**. PR 5c.2-B.1a aterriza un **workspace privado aislado** en `tools/react-e2e-harness/` (sin superficie de edición en `src/`, sin cambios de API/FastAPI/SQLite/extension, sin cambios en `domain/keys.ts` / `infrastructure/store.ts`, sin cambios en `next.config.mjs` / `package.json` de producción, sin captura G4, sin borrado legacy). Pins: Next 16.3.3, React 19.2.8, ReactDOM 19.2.8, `@playwright/test` 1.62.1 (revisión canónica `1234` de chromium-headless-shell gestionada por Playwright — Chromium 151.0.7922.34), Node ≥ 20.9.0. `npm install` genera `tools/react-e2e-harness/package-lock.json` — la **excepción de tamaño de lockfile generado aprobada por el usuario** solo para este workspace aislado (fuente/docs authored ≤ 400 líneas de diff; total authored = 245 LoC entre `package.json` + `next.config.mjs` + `tsconfig.json` + `app/layout.tsx` + `app/page.tsx`). `next.config.mjs` refleja los flags de exportación estática G2 (`output: "export"`, `images.unoptimized: true`, `trailingSlash: false`, `reactStrictMode: true`); `app/layout.tsx` es un título semántico mínimo del arnés (sin réplica de AppShell / chrome, sin `import "./globals.css"`, sin preload de Raleway); `app/page.tsx` monta `FileExplorer` directamente desde `@taxa/research` contra un id de taxon sintético no nulo (`1`) y un `baseUrl` leído desde `NEXT_PUBLIC_HARNESS_BASE_URL` (default `http://127.0.0.1:8765`); `tsconfig.json` declara aliases de path `@taxa/*` seguros que resuelven a `../../src/modules/*/index.ts` (sin reexportación de barrel).
      - **TDD estricto**: sin superficie de test en esta sub-rebanada. **RED** = verificación negativa de contrato de fuente pre-build (`test ! -e tools/react-e2e-harness/app/page.tsx`) PASÓ sobre la fuente previa a `5c.2-B.1a` (arnés ausente). **GREEN** = `npm ci` exit `0` (32 paquetes) + `npm run build` exit `0` (Next 16.3.3 + Turbopack, ~230ms compile, ~670ms TypeScript, 3 páginas estáticas). `out/index.html` (6247 bytes) contiene `<title>Taxa React E2E Harness — FileExplorer mount</title>`, `data-harness-root="react-e2e"`, `data-harness-surface="file-explorer"`, `data-harness-taxon-id="1"`, y el montaje vivo de `FileExplorer` renderizando `data-explorer="loading"` con `aria-busy="true"`. Triangulación: los 4 flags G2 exportados verbatim; `package.json` fija versiones exactas (`next@16.3.3`, `react@19.2.8`, `react-dom@19.2.8`, `@playwright/test@1.62.1` → revisión `1234` de chromium-headless-shell gestionada por Playwright, `engines.node >=20.9.0`); cada alias `@taxa/*` resuelve a `../../src/modules/*/index.ts`; `app/page.tsx` NO importa AppShell / BrowserSurface / `./globals.css` / la página de producción; `app/layout.tsx` carga un título de arnés distinto del `<title>` de producción. **Sin REFACTOR** (andamio mecánico, REFACTOR N/A).
    ...
    ### 2026-09-08 — Evidencia PASS de captura React compuesta canónica registrada (solo sub-puerta canonical-browser E2E React aislada); G4 / G3 Tier-2 / borrado legacy / cutover sin cambios

    - **Alcance (esta entrada, solo evidencia — sin nuevo aterrizaje)**. La captura React compuesta canónica fue impulsada end-to-end a través del orquestador `5c.2-B.1b-ii-c` sobre el arnés ya enviado (`tools/react-e2e-harness/`). **Forma del comando de evidencia**: `make capture-react-e2e OUTPUT_ROOT=<dir>` (equivalente `node tools/react-e2e-harness/scripts/composed-capture.mjs --output-root <dir>`). La invocación corrió SIN `PLAYWRIGHT_CHROMIUM_EXECUTABLE_PATH` en el entorno y SIN flags de override en la CLI; el `buildFn` por defecto del orquestador ejecutó `npm run build` (exit `0`), verificó el acceso a `out/index.html` (fail-closed), inició el servidor API fixture + servidor HTTP de exportación estática en-proceso en puertos loopback asignados por el OS sobre `127.0.0.1`, y luego invocó el `captureFn` por defecto (`run.mjs::capture` → `chromium-driver.mjs::runCapture`) con las opciones de lanzamiento fijadas `{ headless: true }` (sin `channel`, sin `executablePath`). **Pin / metadatos canónicos**: `@playwright/test@1.62.1` exacto → revisión `1234` de chromium-headless-shell gestionada por Playwright (Chromium 151.0.7922.34), coincidiendo con `tools/react-e2e-harness/node_modules/playwright-core/browsers.json` + `~/Library/Caches/ms-playwright/chromium_headless_shell-1234/DEPENDENCIES_VALIDATED`. **Sobre capturado** (persistido como `<OUTPUT_ROOT>/<ISO-timestamp>/evidence.json`; esquema de nivel superior `taxa.react-e2e.run/1` con un payload anidado `taxa.react-e2e.capture/1` bajo `.capture`): `browserExecution = { canonical: true, mode: "pinned-playwright-default", executablePath: null }` (sin marca de override, sin nota diagnóstica), `navigationStatus = 200`, `taxonId = "1"` (no nulo), todas las aserciones del contrato de datos React en VERDE (`data-harness-root="react-e2e"`, `data-harness-surface="file-explorer"`, `data-harness-taxon-id="1"` no nulo, `[data-explorer="ready"]`, ambos slots `[data-pane]`, `input[data-search-input]`, ≥1 `[data-file-path]`); el sobre compuesto `taxa.react-e2e-composed-capture/1` del orquestador es la forma de retorno en memoria y se emite a stdout por `composed-capture.mjs` solo (no se persiste). **Limpieza de artefactos temporales**: el directorio de salida por corrida bajo `<OUTPUT_ROOT>/<ISO-timestamp>/` (conteniendo `evidence.json`) fue verificado y eliminado tras la captura de evidencia; sin `out/` del arnés residual, sin sockets fixture / exportación huérfanos, sin artefactos `playwright/.local-browsers` huérfanos, sin churn de marcadores `DEPENDENCIES_VALIDATED` / `INSTALLATION_COMPLETE`. **Alcance limitado preciso**: esta entrada registra SOLO la sub-puerta de captura canonical-browser E2E React aislada — NO cierra la compuerta más amplia planificada G4 de paridad Playwright + Lighthouse (sigue requiriéndose paridad build de producción vs API de producción + corrida de Lighthouse sobre la app de producción), G3 Tier-2 (sigue con compuerta en el cierre de G4 + G6), borrado legacy `web/*.{html,js,css}` + `tailwind.config.js`, modernización de selectores e2e sobre el árbol de componentes de producción, ni el cutover de PR 3e. **G4 / G3 Tier-2 / G6 / cutover permanecen bloqueadas; ninguna compuerta global se voltea con este registro de evidencia** — la ruta del arnés + pin canónico de Playwright ahora está probada abierta end-to-end dentro del workspace aislado, lo cual desbloquea la autoría del verificador G4 más amplio contra la app de producción, pero G4 en sí sigue bloqueada. Las filas de estado G1 / G2 / G3 Tier-1 / G5 también sin cambios.
    - **Alcance de este intento (vinculante)**: superficies de edición limitadas a los cuatro archivos OpenSpec de solo anexo (`openspec/changes/complete-taxa-frontend-migration/design.md` + `apply-progress.md` + `documents-es/openspec/changes/complete-taxa-frontend-migration/design-es.md` + `apply-progress-es.md`). Sin cambios de código; sin cambios de tests; sin cambios de lockfile; sin cambios de task-list; sin cambios en `src/` de producción; sin cambios en `domain/keys.ts` / `infrastructure/store.ts`; sin cambios en fuentes de `tools/react-e2e-harness/**`; sin cambios en `next.config.mjs` / `package.json` de producción; sin verificador G4 autordado; sin artefacto de build de producción; sin cambios de FastAPI/SQLite/extension; sin commit/push; sin cambios de dependencias. Todos los cambios no comprometidos existentes de entradas previas del registro de cambios (incluyendo las actualizaciones de texto del pin de versión `1.56.0 → 1.62.1` + la anotación de la revisión `1234` de chromium-headless-shell en ambos espejos) se preservan verbatim. **Sin volteo de puerta, sin autoridad de cutover concedida** — las filas de estado G1 / G2 / G3 Tier-1 / G3 Tier-2 / G4 / G5 / G6 y las filas de autoridad de cutover de PR 3e se preservan verbatim desde la entrada previa del registro de cambios.
