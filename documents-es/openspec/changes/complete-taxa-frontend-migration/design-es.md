...
> **2026-09-03 — nota de supersesión de la re-división del
> PR 3c-d (el re-plan de tareas del PR #150 es
> autoritativo; este documento aún refleja solo la
> re-división del CSS de 16 hijos previa a #150)**. El
> PR #150 (`tasks.md` / `tasks-es.md` "3c-d
> unsatisfiability split") re-planea el PR 3c-d
> monolítico previo (animaciones / utilidades +
> paridad final) en tres hijos secuenciales, cada uno
> ≤ 400 líneas authored. La tabla de rebanada de
> sub-PRs y el orden de dependencia de arriba siguen
> siendo la **vista previa a #150**; la
...
>   clases de utilidad (`bg-primary`, `text-on-surface`,
>   `border-outline-variant`,
>   `bg-surface-container-lowest`, `shadow-sm`,
>   `rounded-r-md`, `bg-primary-fixed`,
>   `text-on-primary-fixed`, …) **más cualquier
>   `@keyframes` / `color-mix()` restante
>   (`color-mix()` scoped a componente, reglas
>   emparejadas con utilidad)** — las afinidades
>   globales de estado de `@layer base` se quedan en
>   3c-d. **Sin test de paridad.** Producción
>   permitida: `src/app/globals.css`; test
...
>   consolida los cinco tests enfocados previos
>   (tokens 3c-a / taxonomía 3c-b / research 3c-c /
>   base-resets 3c-d / utilidades 3c-e). El contrato
>   de paridad final queda sin cambios; pertenece
>   solo al PR 3c-f.
>
...
> 3e 16→**18/18**. **3c-d se queda en 6/18** (misma
> rama, reducido). Ramas nuevas: `…-07-3c-e`
> (base `…-06-3c-d`), `…-08-3c-f` (base
> `…-07-3c-e`). El **conteo de 18 hijos** reemplaza
> al de 16; la estrategia `feature-branch-chain` y
> el contrato de "tracker-only targeting `develop`"
> se mantienen. Los presupuestos LoC por sub-PR
> quedan muy por debajo de 400; solo permanece la
> excepción previa de `package-lock.json` regenerado
> de PR 3a. **G4 / G5 / G6 (ahora 17/18 / 15/18 /
> 16/18), el predecesor congelado, el Enfoque A,
> FastAPI/SQLite y los specs por dominio quedan sin
> cambios**. PRs fusionadas 3c-a/#147, 3c-b/#148,
> 3c-c/#149 preservadas. Los cinco hijos CSS
> (3c-a / 3c-b / 3c-c / 3c-d / 3c-e) más el PR 3c-f
> no pueden colapsarse sin violar el presupuesto de
> revisión de 400 líneas por PR.
> **Nota de supersesión solo documental**: sin
> edición de código fuente, sin rebase, sin creación
> de rama nueva en esta revisión; el próximo worktree
> de código recoge el alcance autoritativo desde
> `tasks.md`.
...
## Addenda — 2026-09-08: evidencia PASS de captura React compuesta canónica (solo sub-puerta canonical-browser E2E React aislada); paridad G4 Playwright + Lighthouse / G3 Tier-2 / borrado legacy / cutover sin cambios (solo anexo)

- **Captura React compuesta canónica PASS (esta entrada registra evidencia, no un nuevo aterrizaje — solo sub-puerta canonical-browser E2E React aislada)**. La forma del comando end-to-end fue `make capture-react-e2e OUTPUT_ROOT=<dir>` (equivalente `node tools/react-e2e-harness/scripts/composed-capture.mjs --output-root <dir>`) — sin `PLAYWRIGHT_CHROMIUM_EXECUTABLE_PATH` exportado, sin flags de override en la CLI, arnés construido de cero (`npm run build` exit `0`, `out/index.html` renderizado con `<title>Taxa React E2E Harness — FileExplorer mount</title>`, `data-harness-surface="file-explorer"`, `data-harness-taxon-id="1"`), el servidor API fixture + el servidor HTTP de exportación estática en-proceso (`5c.2-B.1b-ii-a` + `5c.2-B.1b-ii-b`) vinculados en puertos loopback asignados por el OS sobre `127.0.0.1`, y `chromium-driver.mjs` lanzado con las opciones fijadas `{ headless: true }` (sin `channel`, sin `executablePath`). Confirmación de pin: `@playwright/test@1.62.1` exacto (revisión canónica `1234` de chromium-headless-shell gestionada por Playwright — Chromium 151.0.7922.34), coincidiendo con el `playwright-core/browsers.json` local + `~/Library/Caches/ms-playwright/chromium_headless_shell-1234/DEPENDENCIES_VALIDATED`. El artefacto de evidencia persistido `<OUTPUT_ROOT>/<ISO-timestamp>/evidence.json` (esquema de nivel superior `taxa.react-e2e.run/1` con un payload anidado `taxa.react-e2e.capture/1` bajo `.capture`) reporta `browserExecution.canonical = true` (modo `pinned-playwright-default`, `executablePath: null` — sin marca de override, sin nota diagnóstica), `navigationStatus: 200`, `taxonId: "1"` (no nulo), y todas las aserciones del contrato de datos React en VERDE (`data-harness-root="react-e2e"`, `data-harness-surface="file-explorer"`, `data-harness-taxon-id="1"` no nulo, `[data-explorer="ready"]`, ambos slots `[data-pane]`, `input[data-search-input]`, ≥1 `[data-file-path]`); el sobre compuesto `taxa.react-e2e-composed-capture/1` del orquestador es la forma de retorno en memoria y se emite a stdout por `composed-capture.mjs` solo (no se persiste). **Limpieza de artefactos temporales**: el directorio de salida por corrida bajo `<OUTPUT_ROOT>/<ISO-timestamp>/` (conteniendo `evidence.json`) fue verificado y eliminado tras la captura de evidencia; sin `out/` del arnés residual, sin sockets fixture / exportación huérfanos, sin artefactos `playwright/.local-browsers` huérfanos, sin churn de marcadores `DEPENDENCIES_VALIDATED` / `INSTALLATION_COMPLETE`. **Alcance limitado preciso**: esta es SOLO la sub-puerta de captura canonical-browser E2E React aislada — NO cierra la compuerta más amplia planificada G4 de paridad Playwright + Lighthouse (sigue requiriéndose paridad build de producción vs API de producción + corrida de Lighthouse sobre la app de producción), G3 Tier-2 (sigue con compuerta en el cierre de G4 + G6), borrado legacy `web/*.{html,js,css}` + `tailwind.config.js`, modernización de selectores e2e sobre el árbol de componentes de producción, ni el cutover de PR 3e. **G4 / G3 Tier-2 / G6 / cutover permanecen bloqueadas; ninguna compuerta global se voltea con este registro de evidencia** — la ruta del arnés + pin canónico de Playwright ahora está probada abierta end-to-end dentro del workspace aislado, lo cual desbloquea la autoría del verificador G4 más amplio contra la app de producción, pero G4 en sí sigue bloqueada. El espejo en español carga la misma semántica; cualquier deriva se resuelve a favor del inglés.
