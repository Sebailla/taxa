...
| `tests/test_design_system_purity.py` | CSS (PR 3c-iv-barrel) | Guardia de grep sobre `src/modules/design-system/`; sin literales hex fuera del módulo design-system |
...
| 5.5 / 22 | PR 5.5 (reparación de pipeline Tailwind 4 / PostCSS; aterrizada) | NUEVO (reparación post-3c-iii; depende de PR 3c-iii; rama `feat/complete-taxa-frontend-migration-05-5-3c-iv-predecessor`, base `feat/complete-taxa-frontend-migration-05-3c-iii`) | `package.json` (+2 deps: `@tailwindcss/postcss@^4.3.3` + `postcss@^8.5.0`) + `postcss.config.mjs` (nuevo; ESM; registra `@tailwindcss/postcss`) + `package-lock.json` regenerado (`size:exception` aprobada por el usuario; 16 nuevas entradas tailwind/postcss; total 121 paquetes) + `tests/test_toolchain_bootstrap.py` (actualizado: `REQUIRED_DEPS_PRODUCTION` expandido; `FORBIDDEN_LEGACY_DEPS` reducido) + `tests/test_tailwind_build_pipeline.py` (nuevo; 7 casos; `next build` real + aserciones de artefacto CSS compilado) | Nuevo | ~259 authored (≤ 400; −141 LoC de holgura); excepción de lockfile regenerado aprobada por el usuario |
| 5.6 / 22 | PR 5.6 (reparación de paridad estructural DOM↔CSS; aterrizada) | NUEVO (reparación post-3c-iv-predecessor; depende de PR 5.5 + PR 5a + PR 5b; rama `feat/complete-taxa-frontend-migration-05-6-3c-iv-predecessor`, base `feat/complete-taxa-frontend-migration-05-5-3c-iv-predecessor`) | `src/app/globals.css` (reparación CSS-only: 15 hooks estructurales emitidos por React + 9 selectores de estado + 2 reglas de descendientes colapsadas + 1 puente de selector kebab + 5 declaraciones de triangulación de estado visible / encadenable / desplazable; reglas legacy `.tree-row[data-realm="..."]` teñidas por realm + reglas muertas `.kebab-trigger` + reglas muertas `.detail-item .authorship` REMOVIDAS; `.scientific-name` MOVIDA a `@layer components`) + `tests/test_tailwind_4_parity.py` (+37 casos de test de PR 5.6; 3 tests preexistentes de tinte por realm de PR 3c-ii REPROPÓSITOS para afirmar la ausencia de código muerto; constante `TAXONOMY_SELECTORS` actualizada) | Nuevo | ~1,147 a través de dos archivos (491 inserciones + 64 eliminaciones en `src/app/globals.css` + 539 inserciones + 53 eliminaciones en `tests/test_tailwind_4_parity.py`); excepción CSS-only de LoC authored aprobada por el usuario |
| 6 / 22 | PR 3c-iv-barrel (barrel del design-system + Icon/Button + purity test) | tarea 3.2 (rebanada 4-barrel) + tarea 3.3 barrel del design-system del predecesor | `src/modules/design-system/infrastructure/index.ts` (barrel + tokens de tema tipados) + `src/modules/design-system/presentation/{Icon.tsx,Button.tsx}` + `tests/test_design_system_purity.py` | Nuevo | ~120 (≤ 400; -280 LoC de holgura) |
| 7 / 22 | PR 3c-iv-keyframes (cinco `@keyframes` legacy + paridad de `.animate-spin`) | tarea 3.2 (rebanada 4-keyframes) | `src/app/globals.css` (extendido, reglas `@keyframes detail-card-enter` / `detail-card-leave` / `search-pulse-anim` / `materialize-spin` / `toast-slide-in` + utility `.animate-spin`) + `tests/test_tailwind_4_parity.py` (rebanada de `@keyframes`) | Nuevo | ~80 (≤ 400; -320 LoC de holgura) |
| 8 / 22 | PR 3c-iv-viewer (paridad CSS del visor de imagen / video) | tarea 3.2 (rebanada 4-viewer) | `src/app/globals.css` (extendido, `.fex-image-frame` / `.fex-image` / `.fex-image-advisory` / `.fex-video-frame` / `.fex-video-el`) + `tests/test_tailwind_4_parity.py` (rebanada del visor) | Nuevo | ~50 (≤ 400; -350 LoC de holgura) |
| 9 / 22 | PR 3c-iv-settings (paridad CSS de la vista Settings) | tarea 3.2 (rebanada 4-settings) | `src/app/globals.css` (extendido, `.settings-shell` / `.settings-header` / `.settings-list` / `.settings-row*` / `.settings-theme-toggle` / `.settings-action-btn` / `.settings-link-btn`) + `tests/test_tailwind_4_parity.py` (rebanada de Settings) | Nuevo | ~50 (≤ 400; -350 LoC de holgura) |
| 10 / 22 | PR 3c-iv-colors (aliases del namespace `--color-*` de Tailwind + paridad de utility) | tarea 3.2 (rebanada 4-colors) | `src/app/globals.css` (extendido, aliases del namespace `--color-*` de Tailwind + paridad de clases de utilidad legacy) + `tests/test_tailwind_4_parity.py` (rebanada de utility) | Nuevo | ~80 (≤ 400; -320 LoC de holgura) |
| 11 / 22 | PR 3d (Makefile/mount) | tarea 3.4 (porción Makefile) + tarea 3.6 + 3.7 (repoint WEB_DIR + AC-21) | Reescritura de `Makefile::api` (corre `check-runtime.mjs` → `npm run build:web` → `uvicorn … --port 8765`; el `make css` legacy se vuelve shim no-op) + repoint de `api/server.py:54` WEB_DIR + `web/search_urls.js` → `src/data/search-engines.js` + actualización de `open()` de AC-21 + `tests/test_make_api_build.py` + `tests/test_static_mount.py` | Nuevo (fusionado) | ~240 (≤ 400) |
| 12 / 22 | PR 4a | tarea 4.1 + 4.2 | `src/modules/browser-state/{domain/keys.ts, infrastructure/store.ts, index.ts}` + 4 sitios de lectura + 4 de escritura dentro de `useEffect` | Nuevo | ~180 (≤ 400) |
| 13 / 22 | PR 4b | tarea 4.3 + 4.4 | `useSyncExternalStore` detrás de flag `mounted` + aserción Playwright de cero warnings de hidratación | Nuevo | ~90 (≤ 400) |
| 14 / 22 | PR 5a | tarea 5.1 + 5.2 + 5.3 | `src/modules/taxonomy/{domain,application,infrastructure,presentation}` + port de `web/{tree,detail,breadcrumb}.js` | Nuevo | ~280 (≤ 400) |
| 15 / 22 | PR 5b | tarea 5.4 + 5.5 + 5.6 | `src/modules/research/{domain,application,infrastructure,presentation}` + port de `web/{file_explorer,file_viewer,format,keymap}.js` + pin CDN | Nuevo | ~360 (≤ 400) |
| 16 / 22 | PR 5c | tarea 5.7 + 5.8 + 5.9 | Actualizaciones de selectores Playwright + e2e + preservación del contrato `data-*` + borrar `web/*.{html,js,css}` + `tailwind.config.js` | Nuevo | ~200 (≤ 400) |
| 17–19 / 22 | Fase 6a / 6b / 6c (validación) | NUEVO | Reconstrucción de baseline G5 / ensayo de cutover G6 / medición de paridad G4 Playwright + Lighthouse (trabajo de validación; sin código nuevo en `web/**`, handlers de ruta de `api/server.py`, ni `extension/**`) | Nuevo (medición) | ~190 + ~120 medición (≤ 400 cada uno) |
...
- **PR 3c-iv-barrel — barrel del design-system + Icon/Button + purity test** depende del **predecesor de reparación de paridad estructural DOM↔CSS de PR 5.6** (la expansión `@tailwindcss/postcss` + los ganchos estructurales emitidos por React están en su lugar para que el módulo design-system pueda aterrizar limpiamente). **Primer hijo de la sub-secuencia 3c-iv** (según el addendum del replan five-slice de 3c-iv).
- **PR 3c-iv-keyframes — cinco `@keyframes` legacy + paridad de `.animate-spin`** depende de 3c-iv-barrel (módulo design-system + primitivas Icon/Button cargadas para que las reglas de keyframes referencien la superficie design-system viva).
- **PR 3c-iv-viewer — paridad CSS del visor de imagen / video** depende de 3c-iv-keyframes (`@keyframes` + `.animate-spin` vivos para que las referencias `animation:` de los frames del visor resuelvan).
- **PR 3c-iv-settings — paridad CSS de la vista Settings** depende de 3c-iv-viewer (frames del visor vivos para que la vista Settings reuse la misma superficie de tokens).
...
- **PR 5c — e2e + borrar legacy** depende de 5b (todos los
  componentes UI en vivo) y de 3c-iv-colors (el test de
  paridad final de Tailwind 4 está en disco; el CSS inline
  legacy de 1.963 líneas ha sido migrado a `src/app/globals.css`
  de extremo a extremo vía la sub-secuencia de cinco hijos
  3c-iv y está listo para retirarse en PR 5c; **según el
  replan five-slice de 3c-iv, los consumidores CSS finales
  dependen de colors, NO del antiguo PR 3c-iv único**).
...
| `src/app/globals.css` | Creado (PR 3c-i, tokens / base / dark mode) — Tailwind 4 `@import "tailwindcss"` + `@theme` reflejando cada token legacy `:root` / `[data-theme="dark"]` / `--realm-*` + bloque inicial de `@layer base` con resets body / html / `main > :first-child` + selectores `:focus-visible` globales. PR 3c-i **también** añade `import "./globals.css";` a `src/app/layout.tsx` (la corrección del defecto de dependencia — el import vive con el archivo que importa). PR 3c-ii extiende el archivo con selectores de taxonomía; PR 3c-iii extiende con selectores de Search / Folder / global Browser; **PR 3c-iv-keyframes** extiende con cinco reglas `@keyframes` + `.animate-spin`; **PR 3c-iv-viewer** extiende con frames de visor de imagen / video; **PR 3c-iv-settings** extiende con selectores de la vista Settings; **PR 3c-iv-colors** extiende con aliases del namespace `--color-*` de Tailwind | nuevo (3c-i) + extendido (3c-ii / 3c-iii / 3c-iv-keyframes / 3c-iv-viewer / 3c-iv-settings / 3c-iv-colors) |
| `src/modules/design-system/{infrastructure/index.ts, presentation/Icon.tsx, presentation/Button.tsx}` | Creados (PR 3c-iv-barrel, barrel del design-system + Icon/Button + purity test) — barrel de design-system `<Icon>` (envoltorio de glyphs Material Symbols Outlined) + primitiva de layout `<Button>` | nuevos |
