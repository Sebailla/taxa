...
| G6 | `scripts/rehearse_cutover.py` referenciando `design.md::§3.3.6` y el `openspec/changes/migrate-nextjs-tailwind4/cutover-manifest.json` canónico | **andamio PR 1/5 autordado (CLI solo dry-run, rehúsa sin `--dry-run`, nunca ejecuta `verification.command` / `rollback`, escribe atómicamente un `cutover-rehearsal.json` mínimo); validación G6 completa sigue pendiente PRs 2–5** |
