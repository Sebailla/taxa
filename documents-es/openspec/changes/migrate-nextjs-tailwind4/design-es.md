...
| G6 | `scripts/rehearse_cutover.py` + `scripts/rehearse_cutover_path_text.py` referenciando `design.md::§3.3.6` y el `openspec/changes/migrate-nextjs-tailwind4/cutover-manifest.json` canónico | **PRs 1–4/6 mergeadas en `origin/develop` (PR #217 andamio + PR #218 seguridad de ruta + PR #219 normalización del manifiesto + PR #220 validación de esquema fail-closed); PASS de dry-run canónico registrado el 2026-09-11 contra el manifiesto normalizado de 26 consumidores (esta pasada de planificación es la rebanada de ledger documental de G6 en español — PR 6/6)** |
...
| Esquema del registro de consumidor | Cada entrada en `consumers[]` DEBE llevar los siete campos `id` (string estable), `ownership_edge` (debe igualar uno de `edges[].id`), `current_path` (ruta repo-relativa estricta aceptada por `validate_repo_relative_path`; la ruta de runtime / ruta lectora de test actualmente activa contra el borde de ownership elegido), `replacement` (`{status: "unselected" | "selected", path?, note?}` donde `path` es una ruta repo-relativa estricta), `verification`(`{command, expect}`),`activation_status`(`"unselected" | "selected"`),`rollback` (la sentencia exacta de revert que restaura `current_path`). Las anotaciones humanas (llamadas entre paréntesis), las listas de brace-expansion, las enumeraciones de rutas separadas por coma, y los hechos de rango de línea que antes inflaban`current_path` / `replacement.path` DEBEN moverse a campos auxiliares opcionales — `current_paths` (list[str]), `current_path_lines` (str), `current_path_note` (str), `replacement.paths` (list[str]), `replacement.lines` (str), `replacement.path_note` (str) — añadidos solo donde se necesitan y omitidos en caso contrario; su ausencia en un registro NO DEBE tratarse como un campo requerido faltante. El verificador rechaza cualquier registro al que le falte uno de los siete campos requeridos y rechaza cualquier `current_path` / `replacement.path` que falle `validate_repo_relative_path`. |
| Regla de ruta estricta (PR 3/6) | `current_path` y `replacement.path` DEBE cada uno ser una única ruta slash repo-relativa (con sufijo opcional `:line` o `:line:column`) aceptada por el helper merged `validate_repo_relative_path` (PR 2/5 — `scripts/rehearse_cutover_path_text.py`). Paréntesis, brace-expansion `{a,b,c}.js`, enumeraciones de rutas separadas por coma, spans de rango de línea (p. ej. `:77-100`), prefijos de unidad, metacaracteres de shell, y componentes `..` están prohibidos en los campos de ruta estricta; la semántica legible por humanos que codifican DEBE ser llevada verbatim por los campos auxiliares opcionales listados en la fila `Esquema del registro de consumidor` arriba. PR 4/6 enchufará esta regla en el verificador strict-TDD G3 mediante el mismo helper; PR 3/6 normaliza el manifiesto canónico en disco. |
...
| **Disposición (2026-09-11 — paso 1 del camino de cierre G5)** | **Paso 1 completo — la captura de la línea base del legado ahora es reproducible bajo el CLI G5 documentado; la comparación todavía requiere los pasos 2 + 3.** `scripts/measure_hydration.py` se extendió (sin romper el contrato posicional original `validate <artifact>` de PR 1b.3a — códigos de salida 0/1/2/3 preservados verbatim) con un único modo nuevo: **capture** (`--baseline <output> --candidate <root> --iterations N`) que recorre la raíz candidata para bytes fuente `*.html / *.css / *.js` y escribe un artefacto de línea base determinista. El modo **compare** está DIFERIDO al paso (3) del camino de cierre (la captura del lado candidato es el paso (2) y requiere evidencia G2 + G4 que no existe todavía) y NO está implementado en esta pasada. La captura es **hermética** (sin navegador en vivo, sin red, sin servidor FastAPI en vivo) por lo que corre en CI sin un binario de chromium; la misma raíz candidata produce artefactos byte-idénticos (modulo el campo wall-clock `captured_at`, que el test de regresión normaliza). El inventario de pruebas faltantes del camino de cierre (1) se cierra del lado de la línea base: (i) la línea de comando de captura queda registrada verbatim en `provenance.command_line`; (iv) el número de iteraciones queda registrado en `provenance.iterations`; (v) + (vi) el JSON Playwright / Lighthouse crudo **no** se emite (fuera del alcance de la captura hermética); (viii) el esquema de 6 claves existente (`captured_at`, `build`, `route`, `server_shell`, `client_render`, `console_warnings`) se preserva verbatim para que el modo `validate <artifact>` del legado cubra la línea base sin modificación. El campo `provenance` del artefacto escrito por el script solo contiene cuatro claves: `schema` (`taxa.g5-hydration-baseline/1`), `command_line`, `iterations`, `captured_at` — NO contiene `environment`, NO contiene `deterministic`, NO contiene `raw_capture_emitted`. Cualquier revisión previa que afirme esos campos quedó retirada en la pasada de reducción de alcance. **Cobertura de tests** — 6 tests enfocados en `tests/test_hydration_timing.py` (`capture_writes_baseline_artifact`, `capture_is_byte_reproducible`, `capture_fails_on_missing_candidate`, `capture_validates_written_artifact`, `capture_requires_candidate`, `exits_one_on_no_arguments`) todos verdes; los 11 tests originales PR 1b.3a siguen verdes (contrato `validate` posicional sin cambios). **Verdad preservada** — el paso (2) del camino de cierre G5 (captura del lado candidato contra el artefacto de build del Enfoque elegido) sigue bloqueado por G2 + G4; el paso (3) del camino de cierre G5 (unión de línea base + candidato a través del comando G5) sigue bloqueado por el paso (2); el paso (4) del camino de cierre G5 (aserción de umbrales ±10 %) sigue bloqueado por el paso (3); la ubicación de la línea base `docs/baselines/legacy-web-2026-08-26.json` queda **gitignored** por `.gitignore` (`docs/baselines/`); ningún artefacto preexistente viaja en el repo — la línea base es reproducible bajo demanda (misma entrada → mismos bytes, modulo `captured_at`) de modo que un clon limpio puede regenerarla ejecutando `scripts/measure_hydration.py --baseline docs/baselines/legacy-web-2026-08-26.json --candidate <legacy-server-root> --iterations 10` contra `tools/g3-legacy-fixture/web/` sin contactar ningún sistema externo; sin activación de FastAPI (sin repoint de `WEB_DIR`, sin cutover atómico, sin cambio en `api/server.py` / Makefile / extensión / API / fuente de producto); los Enfoques A / B / C quedan sin seleccionar; G3 Nivel-1 sigue APROBADO, Nivel-2 sigue bloqueado por G4 + G5 + G6; G4 sigue bloqueada (verificador no autordado todavía); **G6 PASS registrado el 2026-09-11 (dry-run canónico sobre el manifiesto normalizado de 26 consumidores; PR #217 + #218 + #219 + #220 mergeadas en `origin/develop`)**, ejecución atomic-cut de Nivel-2 / ensayo de rollback sigue acoada por evidencia; el artefacto de línea base del legado NO es sustituto de una captura Playwright real — es la forma reproducible en disco del paso (1) del camino de cierre §3.3.5; la captura Playwright real reemplazará los números sintéticos con los medidos en PR3d y dejará este artefacto atrás para reproducibilidad. |
| **Contrato de reproducibilidad (2026-09-11)** | **La captura del paso 1 del camino de cierre G5 es byte-determinista** para una raíz candidata dada. Cada valor excepto `captured_at` (y el `provenance.command_line` registrado que refleja legítimamente el argv provisto por el usuario) es reproducible: misma raíz candidata + mismo `--iterations` → mismo artefacto (modulo el timestamp de wall-clock). El contrato queda pineado por `tests/test_hydration_timing.py::test_measure_hydration_capture_is_byte_reproducible` que corre la captura dos veces contra la misma raíz y aserta igualdad de bytes tras normalizar `captured_at` + `provenance.command_line` fuera de la comparación. Una regresión futura que introduzca no-determinismo (orden de iteración aleatorio, `time.time()` en el cuerpo del artefacto, claves de dict no ordenadas, etc.) queda capturada por este test. |
| **Logs de prueba del bridge (Rebanada 4 del CLI orquestador G5)** | **Autorizado.** El orquestador Python (`scripts/run_g5_orchestration.py`) es dueño de los logs de prueba del bridge; el bridge Node (`tools/g4-capture/scripts/g5_raw_lhr_bridge.mjs`) queda libre de efectos secundarios. Nuevo flag CLI `--bridge-log-dir` por defecto `<out>/raw/bridge-logs`; en cada invocación del bridge el CLI persiste atómicamente `bridge-NN.json` (NN = iteración de dos dígitos) llevando `schema=taxa.g5-bridge.proof-log/1`, `argv`, `cwd`, `timestamp` (ISO-8601 UTC), `timeout_s`, `duration_ms`, `exit_code`, `timed_out`, `stdout`, y `stderr`. Los logs de ejecuciones previas se rotan en la primera escritura (cada fichero nuevo se produce vía fichero temp + `os.replace`). El fallo de escritura del log NO es bloqueante — se emite un aviso distinto a stderr `[run_g5_orchestration] bridge-log write failed: ...` y se preserva el contrato de éxito / sentinel / BridgeError del bridge. Tests herméticos cubren éxito, exit distinto de cero, timeout, fallo de escritura, nombre de fichero / rotación, y threading CLI / validación / aislamiento de dry-run. **Resuelve los ítems de prueba faltantes (i) línea de comando de captura (argv), (ii) log stdout/stderr de captura, (iv) número de iteraciones, y (vi) envoltorio JSON Lighthouse crudo** vinculando cada invocación del bridge por iteración a un log de prueba JSON en disco bajo `<out>/raw/bridge-logs/`. Sigue falta para la plena reproducibilidad G5: (iii) entorno de captura (versión Node, pin SHA256 de chromium, versión FastAPI, hash de ruta FastAPI), (v) JSON Playwright crudo, (vii) fila de delta candidato vs línea base, (viii) coincidencia con el esquema de la línea base del legado. |

#### §3.3.6 G6 — ensayo de corte

| Campo | Valor |
| --- | --- |
| Productor | `scripts/rehearse_cutover.py` + `scripts/rehearse_cutover_path_text.py` (PRs 1–4/6 mergeadas en `origin/develop`: PR #217 andamio + CLI solo dry-run; PR #218 validadores de ruta / texto de shell; PR #219 normalización de rutas del manifiesto canónico; PR #220 validación de esquema fail-closed delegando las reglas de ruta + shell a los helpers de PR #218 — sin lógica duplicada) |
| Comando | `python3 scripts/rehearse_cutover.py --manifest openspec/changes/migrate-nextjs-tailwind4/cutover-manifest.json --out <out> --dry-run` (`--dry-run` es REQUERIDO — no se ofrece variante `--execute`; `verification.command` y `rollback` NUNCA se invocan) |
| Artefacto | `<out>/cutover-rehearsal.json` (PR #217 emite un artefacto mínimo con `manifest_path`, `manifest_sha256`, `validated_at`, `mode: "dry-run"`, `out_dir`, `consumer_count`, `verification_executed: false`, `rollback_executed: false`; PR #220 lo extiende con `consumer_ids[]` (uno por consumidor, orden de declaración) + `validation_errors[]` (vacío en éxito) tras la rama de esquema fail-closed) |
| Umbral | en éxito: `cutover-rehearsal.json` se emite atómicamente bajo `<out>/` vía temp-file + `os.replace`; `consumer_count` iguala la longitud de `consumers[]` del manifiesto canónico; cada consumidor en orden de declaración queda nominado en `consumer_ids[]` con `id`, `current_path`, `replacement.path`, `verification.{command, expect}`, `rollback` pasando `validate_repo_relative_path` + `parse_shell_text` (helpers de PR #218); `validation_errors` es `[]`; `verification_executed` es `false`; `rollback_executed` es `false`; `manifest_sha256` es estable entre ejecuciones consecutivas del verificador. Ramas fail-closed: `consumers` faltante / no-lista ⇒ `EXIT_SCHEMA = 4`, sin artefacto; cualquier consumidor al que le falte un campo requerido ⇒ `EXIT_SCHEMA = 4`, sin artefacto; `id` duplicado ⇒ `EXIT_SCHEMA = 4`, sin artefacto; cualquier `current_path` / `replacement.path` que falle `validate_repo_relative_path` ⇒ `EXIT_SCHEMA = 4`, sin artefacto; cualquier `verification.command` / `rollback` que falle `parse_shell_text` ⇒ `EXIT_SCHEMA = 4`, sin artefacto; `--dry-run` omitido ⇒ `EXIT_USAGE = 2`, sin artefacto. **G6 queda `APROBADO para dry-run canónico; ejecución atomic-cut de Nivel-2 / ensayo de rollback sigue acoada por evidencia`** — el script de ensayo es solo dry-run por diseño (no se ofrece variante `--execute`); la selección atomic-cut sigue acoada por PASS de G2 + G4 + G5 + G6 (aquí: G6 == PASS de dry-run) y NO está en esta rebanada. |
| **Disposición (2026-09-11 — PASS de dry-run canónico G6)** | **PASS — dry-run canónico sobre el manifiesto normalizado de 26 consumidores `openspec/changes/migrate-nextjs-tailwind4/cutover-manifest.json` sale `0` y emite atómicamente un artefacto completo `cutover-rehearsal.json` bajo `<out>/`, con `consumer_count = 26`, 26 `consumer_ids[]` únicos en orden de declaración, `validation_errors = []`, `verification_executed = false`, `rollback_executed = false`, `mode = "dry-run"`, `validated_at` ISO-8601 UTC, `manifest_path` coincidiendo con el manifiesto canónico machine-readable, y `manifest_sha256` coincidiendo con el hash del manifiesto canónico en disco (estable entre ejecuciones consecutivas del verificador).** El andamio mergeado — **PR #217 (PR 1/6)** `scripts/rehearse_cutover.py` + `tests/test_rehearse_cutover.py` (CLI solo dry-run, rehúsa sin `--dry-run`, escritura atómica vía temp-file + `os.replace`, códigos de salida `EXIT_OK = 0` / `EXIT_USAGE = 2` / `EXIT_SCHEMA = 4` / `EXIT_IO = 5`); **PR #218 (PR 2/6)** `scripts/rehearse_cutover_path_text.py` + `tests/test_rehearse_cutover_path_text.py` (helpers `validate_repo_relative_path` + `parse_shell_text` puro stdlib); **PR #219 (PR 3/6)** normalización de rutas del `cutover-manifest.json` canónico + tests de triangulación de seguridad de rutas; **PR #220 (PR 4/6)** validación de esquema fail-closed de `scripts/rehearse_cutover.py` (delega reglas de ruta + shell a los helpers de PR #218, junta todos los errores vía `_validate_manifest`, registra cada problema a stderr, sale `EXIT_SCHEMA = 4`, no emite artefacto) — está en disco en el HEAD `dd1c218` de `origin/develop` y el dry-run canónico se ejecuta contra ese HEAD sin fixtures de test. **Tests de aprobación** — `python3 -m pytest tests/test_rehearse_cutover.py tests/test_rehearse_cutover_path_text.py -q` → **137 pasaron en 1.40s** (31 en `tests/test_rehearse_cutover.py` + 106 en `tests/test_rehearse_cutover_path_text.py`); el pin canónico de happy-path `tests/test_rehearse_cutover.py::test_canonical_happy_path_emits_artifact_for_all_26_consumers` aplica `r.returncode == 0`, `(out / "cutover-rehearsal.json").is_file()`, 26 `consumer_ids` únicos, `validation_errors` vacío, `consumer_count == 26`, `verification_executed is False`, `rollback_executed is False`. **Línea de comando canónica** — `python3 scripts/rehearse_cutover.py --manifest openspec/changes/migrate-nextjs-tailwind4/cutover-manifest.json --out <out> --dry-run` con `cwd = repo-root`; sale `0` contra el manifiesto canónico normalizado. **Estabilidad** — la rama de esquema fail-closed de PR #220 junta todos los errores de esquema vía `_validate_manifest`, registra cada error a stderr, sale `EXIT_SCHEMA = 4`, y no emite `cutover-rehearsal.json`; los helpers `parse_shell_text` + `validate_repo_relative_path` de PR #218 descartan rutas absolutas, componentes `..`, caracteres de control / metacaracteres, shell vacío / nulo, y cadenas `verification.command` / `rollback` con comillas desbalanceadas antes de cualquier paso posterior. **Nota de riesgo** — el PASS de dry-run canónico es un registro de evidencia **solo de planificación** para la puerta §3.3.6; **G6 sigue siendo solo dry-run por diseño** (no se ofrece variante `--execute` — `verification.command` / `rollback` nunca se invocan; por tanto el ensayo de rollback sigue **planificado, no ejercitado**, y la selección atomic-cut sigue acoada por PASS de G2 + PASS de G4 + PASS de G5, ninguno de los cuales constituye ejecución de `verification.command` contra el artefacto de build de un Enfoque elegido). **Verdad preservada** — PASS de G2 registrado el 2026-08-30 (sin cambios); G3 **APROBADO** para Nivel-1, **NO APROBADO** para Nivel-2 (la atomic-cut de Nivel-2 sigue acoada por G4 + G5 + G6, y la atomic-cut requiere un Enfoque A / B / C elegido, ninguno seleccionado); G4 sigue `bloqueada` (verificador no autordado); G5 **paso 1 del camino de cierre completo**, pasos 2 + 3 siguen bloqueados; los Enfoques A / B / C siguen sin seleccionar; sin activación de FastAPI (sin repoint de `WEB_DIR`, sin cutover atómico, sin cambio en `api/server.py` / Makefile / extensión / API / fuente de producto); el `cutover-manifest.json` canónico queda sin cambios (26 consumidores de §3.1, Nivel-1 `selected`, Nivel-2 sin seleccionar); el `CONSUMER-READINESS.json` previamente emitido (si lo hay) queda sin cambios. **Disposición de G6: `APROBADO para dry-run canónico; ejecución atomic-cut de Nivel-2 / ensayo de rollback sigue acoada por evidencia`**; ningún fichero fuente / tests / scripts / config / Makefile / extensión / API / fuente de producto tocado, comiteado, o pusheado en esta pasada; espejo en inglés actualizado en lockstep — los docs en español (y la traducción espejo `documents-es/openspec/changes/migrate-nextjs-tailwind4/apply-progress-es.md`) SI se actualizan en esta pasada (PR 6/6, espejo en español de PR #221); sin commit, push, ni apertura de PR abiertos en esta pasada.
| **Disposición (2026-09-13 — artefacto canónico de dry-run de G6 rebanada 5, evidencia de cierre delimitada)** | **PASS del dry-run canónico registrado (evidencia de cierre delimitada); ejecución atomic-cut de Nivel-2 / ensayo de reversión TODAVÍA NO SE EJECUTA.** Según la tarea padre, el `cutover-rehearsal.json` canónico queda capturado en esta rama (`docs/g6-bounded-closure`) en el commit pin-del-artefacto-canónico `66a0015` (el merge del trabajo de rebanada-4 de #220, funcionalmente equivalente al `e1fc5a4` de la rama fuente `feat/g6-rehearsal-schema-validation-2`) vía `python3 scripts/rehearse_cutover.py --manifest openspec/changes/migrate-nextjs-tailwind4/cutover-manifest.json --out /tmp/g6-bounded-closure-docs-dry-run --dry-run` con `cwd = repo-root`; sale `0`; `--dry-run` es REQUERIDO (no se ofrece ninguna variante `--execute`). El artefacto canónico se escribe atómicamente a `/tmp/g6-bounded-closure-docs-dry-run/cutover-rehearsal.json` (gitignored, reproducible bajo demanda — mismos bytes del manifiesto canónico → mismo hash del artefacto modulo `validated_at` de reloj de).). Claves del esquema (canónicas, exactamente 10, congeladas por el pin `EXPECTED_KEY_SET` de la rebanada 4): `consumer_count`, `consumer_ids`, `manifest_path`, `manifest_sha256`, `mode`, `out_dir`, `rollback_executed`, `validated_at`, `validation_errors`, `verification_executed`. NO existe `out_sha256`, NO existe `tier_2_passed`, NO existe `g3_tier2_passed`, NO existe `g4_passed`, NO existe `g5_passed`, NO existe `cutover_complete`, NO existe `rollback_rehearsed`, NO existe `atomic_cut_executed`, NO existe `fastapi_activated`, NO existe `web_dir_repointed` (pin de evidencia delimitada — el artefacto es la autodescripción del script para el dry-run canónico SOLAMENTE; las claves de Nivel-2 / finalización-del-cutover están explícitamente ausentes para que el artefacto no pueda leerse erróneamente como cierre de Nivel-2 / cutover). **Hechos del artefacto canónico** (verbatim, capturado el 2026-09-17 en el dry-run canónico adaptado de PR #258): `manifest_path = "openspec/changes/migrate-nextjs-tailwind4/cutover-manifest.json"`; `manifest_sha256 = "3d228ebe5a98081ace5cf088858c0e7fe2b560c77815e5d68e5a01fb375e0c5d"` (coincide con `shasum -a 256` del manifiesto canónico en disco, estable entre ensayos independientes); `validated_at = "2026-09-17T08:51:44.152763+00:00"` (ISO-8601 UTC); `mode = "dry-run"`; `consumer_count = 26`; `consumer_ids[]` = 26 IDs únicos de consumidores en orden de declaración canónico (21 `mount-*` §3.1.1 + 5 `search-urls-*` §3.1.2); `validation_errors = []`; `verification_executed = false`; `rollback_executed = false`. **Tests que respaldan el PASS** — `python3 -m pytest tests/test_rehearse_cutover.py tests/test_rehearse_cutover_path_text.py -q` → **149 pasaron en ~1.86s** (43 en `tests/test_rehearse_cutover.py` — 31 tests originales PR 1/6–PR 4/6 + 10 pines canónicos del artefacto de la rebanada 4 + **2 nuevos pines de evidencia delimitada de la rebanada 5**: `test_canonical_artifact_bounded_evidence_no_tier2_or_cutover_markers` + `test_canonical_artifact_bounded_evidence_dry_run_only_no_execute_marker`; **106** en `tests/test_rehearse_cutover_path_text.py`), todos en verde en esta rama. Sin skips; sin nuevos fallos. **Lo que este PASS NO afirma** — (i) NO afirma PASS de G6 para la ejecución atomic-cut de Nivel-2 / ensayo de reversión (Nivel-2 sigue acoado por el artefacto de build del Enfoque A / B / C elegido + PASS de G4 + línea base reproducible de G5). (ii) NO afirma PASS de Nivel-2 de G3, PASS de G4, ni PASS de los pasos 2 + 3 del camino de cierre de G5. (iii) NO afirma la finalización del cutover — sin repoint de `WEB_DIR`, sin cutover atómico, sin cambio en `api/server.py` / Makefile / extensión / API / fuente de producto. (iv) NO altera el `cutover-manifest.json` canónico; el `cutover-rehearsal.json` canónico queda **no** comiteado al repo (vive bajo `/tmp`, gitignored, reproducible bajo demanda). El artefacto es la evidencia de la puerta §3.3.6 para el dry-run canónico SOLAMENTE; **no** es un artefacto de Nivel-2 / finalización-del-cutover. |
| **Camino de cierre hacia adelante** | (1) Autordar la superficie `--execute` de G6 + el ensayo de reversión de Nivel-2 una vez que se seleccione el Enfoque A / B / C Y G4 APRUEBE Y los pasos 2 + 3 del camino de cierre de G5 cierren (el artefacto de dry-run canónico en `/tmp/g6-bounded-closure-docs-dry-run/cutover-rehearsal.json` es la evidencia de la puerta §3.3.6 para el dry-run canónico SOLAMENTE; una emisión de Nivel-2 reemplaza (o aumenta) el artefacto de dry-run canónico con preparación atomic-cut, evidencia de ensayo de reversión, y un `out_sha256` de la raíz del artefacto servido); (2) aterrizar el harness de paridad Playwright + Lighthouse y producir informes de paridad según §3.3.4 (G4); (3) solo después de que G4 + pasos 2 + 3 del camino de cierre de G5 + Nivel-2 de G6 cierren, llevar cada consumidor a Nivel-2 `selected` contra el artefacto de build del Enfoque elegido y re-ejecutar el verificador G3 para evaluación de Nivel-2; (4) la emisión de Nivel-2 reemplaza (o aumenta) la emisión de Nivel-1 con reemplazos de ruta nueva y preparación atomic-cut. Hasta que (1)–(3) cierren, **G6 queda `PASS del dry-run canónico registrado (evidencia de cierre delimitada); ejecución atomic-cut de Nivel-2 / ensayo de reversión TODAVÍA NO SE EJECUTA`**. |
 |

---

`status: complete (rebanada PR 2a; alcance de frontera PR3a con plan productor/umbral de evidencia G2–G6 añadido — ninguna frontera seleccionada para el Enfoque A / B / C; contrato G2 definido en §3.3.2.1 con **cuatro** correcciones explícitas del mantenedor aplicadas contra el contrato de salida verificado de Next.js 16.3.3 / Turbopack: (1)`size:exception` SOLO para `tools/g2-candidate/package-lock.json`, solo después de que`npm ci` salga 0 contra el `package.json` local del candidato, ningún otro fichero generado exceptuado; (2) staging post-build de manifiestos atómico desde `<candidate-root>/.next/` → `<candidate-root>/out/.next/` — `build-manifest.json` requerido (su ausencia es un fallo de clase faltante), `app-build-manifest.json` opcional y nunca un fallo de clase faltante (copia best-effort registrada como `staged` / `not_emitted`); (3)`index.html` única entrada HTML de ruta-de-aplicación normal, `404.html`/`500.html` exenciones de página de error explícitamente permitidas clasificadas bajo una clase de activo `error_pages` separada; (4) **corrección del contrato de salida de Next 16 / Turbopack** — clase **CSS** requerida = uno-o-más `*.css` no vacíos bajo `out/_next/static/chunks/**` (no bajo `out/_next/static/css/`); clase **JS** requerida = uno-o-más`*.js` no vacíos bajo `out/_next/static/chunks/**` (sin requisito del subdirectorio `chunks/app/`); PASS de G2 registrado el 2026-08-30 desde el árbol limpio`taxa-worktrees/migrate-nextjs-g2-evidence-capture` sobre `develop@a74289b` (build finalizado `2026-08-30T18:11:02Z`, Node`v26.8.1`, inventario en`taxa-worktrees/migrate-nextjs-g2-evidence-capture/tools/g2-candidate/out/BUILD-INVENTORY.json` con `missing_classes[]` vacío, todas las clases requeridas de ruta-de-aplicación presentes, `build-manifest.json` en staging a 607 bytes / sha256 `f52f7edd901e373a2a24a4ecf8ba61c96ad227093c6440dc4a3a6ca58a92f2a3`,`app-build-manifest.json` opcional `not_emitted`, 14 tests enfocados del verificador + 34 del candidato pasan, log de build capturado — la advertencia de múltiples lockfiles no es bloqueante); línea base del legado de G5 dispuesta como irreproducible según auditoría §3.3.5 del 2026-08-30; contrato G3 definido en §3.3.3.1 con manifiesto machine-readable canónico en`openspec/changes/migrate-nextjs-tailwind4/cutover-manifest.json` autorado el 2026-08-30 que enumera los 26 consumidores de §3.1 (21 §3.1.1 mount web FastAPI + 5 §3.1.2 web/search_urls.js) con IDs estables, `current_path`,`ownership_edge`,`replacement`,`verification`,`activation_status`,`rollback`; **selección legacy pre-cut (Nivel-1) autorada el 2026-08-30 para cada consumidor** — cada consumidor lleva`activation_status: selected` y `replacement.status: selected` contra su `current_path` del legado en disco; `all_replacements_unselected` llevado a `false`; **PASS de Nivel-1 de G3 registrado el 2026-08-30 vía merge PR #109 + PR #111 + PR #115 + PR #116** — comando canónico`python scripts/verify_consumers.py --manifest openspec/changes/migrate-nextjs-tailwind4/cutover-manifest.json --out <build-root> --serve --venv <repo-root>/.venv/bin/python --fixture-web-root <repo-root>/tools/g3-legacy-fixture/web --repo-root <repo-root>`; los **26 / 26** consumidores de §3.1 ejecutan`verification.command`contra el fixture controlado (`tools/g3-legacy-fixture/web/` servido por `python -m http.server` en un puerto libre aislado) con aplicación fail-closed de forma HTTP vía `tools/g3-legacy-fixture/scripts/check_http_status.py`; cada uno sale`0`;`<build-root>/CONSUMER-READINESS.json` se emite atómicamente con `manifest_path`,`manifest_sha256`,`node_version ≥ 20.9.0`,`verified_at`,`exit_code = 0`, cada`consumers[].status = "ready"`,`unselected_count = 0`,`failed_verifications[]` vacío, `activation_complete = true`; **G3 APRUEBA para Nivel-1 (preparación legacy pre-cut)**; **G3 NO aprueba para Nivel-2** — Nivel-2 (selección atomic-cut contra el artefacto de build del Enfoque A / B / C elegido) sigue acoado por PASS de G4 + G5 + G6; los Enfoques A / B / C quedan sin seleccionar; la pasada atomic-cut de Nivel-2 aterriza en PR3d/PR3e después de que G4 / G5 / G6 cierren; el contrato §3.3.3.1 define un **modelo de selección de dos niveles** — Nivel-1 (APROBADO 2026-08-30, legacy pre-cut, sin PASS de G2/G4/G5/G6 requerido) Y Nivel-2 (atomic-cut, acoado por PASS de G4/G5/G6, NO en esta pasada);`fail_closed_summary` y `verifier_contract_summary`actualizados con umbrales de Nivel-1 / Nivel-2; **fail-closed (Nivel-1 + Nivel-2)**: el verificador G3 (`scripts/verify_consumers.py`, AUTORIADO en disco vía PR #109 + PR #111 + PR #115 + PR #116; en`scripts/verify_consumers.py` + tests en `tests/test_verify_consumers.py`) no emite ningún`CONSUMER-READINESS.json` válido mientras CUALQUIER `verification.command` salga no-cero O mientras CUALQUIER puerta de G4/G5/G6 falle (Nivel-2) O mientras falte CUALQUIER campo requerido O mientras la versión Node esté por debajo de `20.9.0` O mientras las expectativas con forma HTTP carezcan del helper `check_http_status.py` (fail-closed PR #115); G4 sigue bloqueada (verificador no autordado); **G6 PASS registrado el 2026-09-11 (esta pasada de planificación — rebanada de ledger documental en español — PR 6/6)** — dry-run canónico sobre el manifiesto normalizado de 26 consumidores `openspec/changes/migrate-nextjs-tailwind4/cutover-manifest.json` sale `0` y emite atómicamente un artefacto completo `cutover-rehearsal.json`(`consumer_count = 26`, 26`consumer_ids[]` únicos, `validation_errors = []`,`verification_executed = false`,`rollback_executed = false`,`mode = "dry-run"`); **137 tests enfocados pasan** (`tests/test_rehearse_cutover.py` 31 / `tests/test_rehearse_cutover_path_text.py` 106); el andamio mergeado es PR #217 + #218 + #219 + #220 (andamio + seguridad de ruta + normalización del manifiesto + validación de esquema fail-closed) en el HEAD `dd1c218` de `origin/develop`— G6 queda **`APROBADO para dry-run canónico`**; **ejecución atomic-cut de Nivel-2 / ensayo de rollback sigue acoado por evidencia** (no se ofrece variante`--execute` por diseño; `verification.command` / `rollback` nunca se invocan; el artefacto de ensayo es una emisión solo de planificación contra el contrato canónico de dry-run, no una evidencia de activación); ningún fichero fuente / tests / scripts / config / Makefile / extensión / API / fuente de producto tocado, comiteado, o pusheado en esta pasada; G5 **paso 1 del camino de cierre completo** (captura de línea base del legado reproducible bajo el CLI G5 documentado según la fila §3.3.5 `Disposición (2026-09-11 — paso 1 del camino de cierre G5)`;`scripts/measure_hydration.py` extendido solo con modo capture hermético (modo compare DIFERIDO al paso 3 del camino de cierre); `docs/baselines/` gitignored; 6 tests enfocados verdes); G5 **pasos 2 + 3 del camino de cierre siguen bloqueados** (la captura del lado candidato requiere evidencia G2 + G4; unir línea base + candidato a través del comando G5 requiere el paso 2; asertar los umbrales ±10 % requiere el paso 3); G5 **disposición**: `paso 1 del camino de cierre completo; comparación todavía no intentada (pasos 2 + 3 pendientes)`; exportación estática sin seleccionar (los Enfoques A / B / C quedan sin seleccionar); sin activación de FastAPI (sin repoint de`WEB_DIR`, sin cutover atómico, sin cambio en`api/server.py` / Makefile / extensión / API / fuente de producto); **disposición de G6: `PASS del dry-run canónico registrado el 2026-09-13 (evidencia de cierre delimitada); ejecución atomic-cut de Nivel-2 / ensayo de reversión TODAVÍA NO SE EJECUTA`** (artefacto canónico capturado en`/tmp/g6-bounded-closure-docs-dry-run/cutover-rehearsal.json` según la fila §3.3.6 `Disposición (2026-09-13 — artefacto canónico de dry-run de G6 rebanada 5, evidencia de cierre delimitada)`; 26 / 26 IDs de consumidores de §3.1 enumerados,`validation_errors` vacío, `verification_executed: false`,`rollback_executed: false`,`mode: "dry-run"`; **disposición de G3:`APROBADO para Nivel-1; NO APROBADO para Nivel-2`**; la pasada atomic-cut de Nivel-2 aterriza en PR3d/PR3e después de que G4 / G5 / G6 cierren; ninguna frontera seleccionada más allá del PASS de G2; ningún fichero fuente / tests / scripts / config / Makefile / extensión / API / fuente de producto tocado, comiteado, o pusheado en esta pasada; espejo en inglés actualizado en paralelo)`
