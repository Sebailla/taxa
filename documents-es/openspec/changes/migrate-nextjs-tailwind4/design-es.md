...
| Umbral | primer render server-shell dentro de ±10 % de la línea base del legado; latencia de interacción (pestañas clave / dropdown búsqueda) dentro de ±10 % de la línea base del legado; tamaño de bundle dentro del umbral declarado (total JS + CSS sin comprimir del candidato ≤ total de la línea base × 1,10; definición completa en la fila Puerta de bundle abajo) |
...
| **Candidato G2 provisional (existente, sólo diagnóstico)** | El candidato de la puerta de bundle es el shell de diagnóstico G2 existente en `tools/g2-candidate/` (raíz de build autorizada sin activación según §3.3.2.1 fila 1). Es **solo evidencia de infraestructura provisional** — existe para captura diagnóstica, **NO** como selección del Enfoque A / B / C y **NO** como candidato para activación de FastAPI. La salida de build emitida del candidato (`<candidate-root>/out/`) es de sólo-lectura para medición; montarla bajo los `StaticFiles` de FastAPI en `127.0.0.1:8765` sigue siendo una decisión separada G3+G6 (bloqueada). |
| **Puerta de bundle (provisional, sólo planificación)** | La línea de tamaño de bundle previamente TBD de la fila Umbral de G5 queda ahora definida (provisional, sólo planificación) como: **total emitido JS + CSS sin comprimir del candidato ≤ total JS + CSS sin comprimir de la línea base del legado × 1,10** (≤ 10 % de delta sobre el peso combinado del bundle sin comprimir). Peso JS/CSS del candidato = suma de bytes sobre cada fichero `*.js` no vacío bajo `<candidate-root>/out/_next/static/chunks/**` + cada fichero `*.css` no vacío bajo la misma ruta (co-ubicado según §3.3.2.1 `Clases de activos requeridas (ruta-de-aplicación)` + contrato de **cuatro** correcciones). Línea base = total JS/CSS del legado en `web/` desde la captura reproducible del legado (paso 1 del camino de cierre). El 10 % de delta es una **puerta de planificación provisional**; **NO** constituye selección de Enfoque, activación de FastAPI, ni aceptación del producto migrado. |
| **Disponibilidad de infraestructura de captura (existente, provisional)** | Existente en disco para **uso sólo de diagnóstico / captura** (provisional, **NO** promueve ninguna puerta a PASS): (a) `tools/g4-capture/` (árbol de captura G4/G5 aterrizado vía PRs #131–#140 sobre `docs/g5-provisional-candidate` en `3cee69b` — fixture de hidratación controlado, precondiciones de captura, muestras Playwright crudas, cargas Lighthouse crudas, publicación atómica, latencia de preparación, puente raw→publicado, ciclo de vida controlado del legado, orquestación de captura); (b) `tools/g3-legacy-fixture/web/` + `tools/g3-legacy-fixture/scripts/check_http_status.py` (fixture controlado G3 + helper fail-closed de forma HTTP, PRs #113–#116); (c) `tools/g2-candidate/` (shell de diagnóstico G2 según §3.3.2.1 fila 1). Estas herramientas están disponibles para captura diagnóstica pero **ninguna de G2 Nivel-2, G3 Nivel-2, G4, G5, o G6 se promueve a PASS** por esta reconciliación. G5 sigue `bloqueada — línea base no reproducible; comparación no intentada` según auditoría §3.3.5; G4 y G6 siguen `bloqueadas — verificadores no autordos`. |
| **No-objetivos (alcance provisional)** | Esta pasada de reconciliación **NO**: (i) acepta el producto migrado final (ningún reclamo de aceptación — solo evidencia de infraestructura provisional); (ii) selecciona Enfoque A / B / C o exportación estática (la selección de Enfoque sigue acoada por evidencia por G2 + G3 Nivel-2 + G4 + G5 + G6); (iii) transfiere ownership de FastAPI ni realiza un cutover (sin repoint de `WEB_DIR`, sin cutover atómico, sin cambio en `api/server.py` / Makefile / extensión / API / fuente de producto); (iv) promueve paridad de comportamiento de G4 a PASS (el harness Playwright + Lighthouse sigue bloqueado según §3.3.4); (v) promueve ensayo de corte de G6 a PASS (`scripts/rehearse_cutover.py` sigue sin autorarse según §3.3.6). |
...
+ **Pasada de reconciliación provisional G5 (2026-08-30, esta pasada)** — `design-es.md::§3.3.5` gana cuatro filas nuevas (Candidato G2 provisional + Puerta de bundle + Disponibilidad de infraestructura de captura + No-objetivos) que definen el **alcance provisional**: `tools/g2-candidate/` como candidato diagnóstico (NO un candidato para activación de FastAPI, NO una selección de Enfoque); puerta de bundle definida como `total emitido JS + CSS sin comprimir del candidato ≤ línea base del legado × 1,10` (provisional, sólo planificación); infraestructura de captura G4/G5 existente (`tools/g4-capture/` aterrizada vía PRs #131–#140 sobre `docs/g5-provisional-candidate` en `3cee69b`) + fixture controlado G3 (`tools/g3-legacy-fixture/`, PRs #113–#116) + shell de diagnóstico G2 (`tools/g2-candidate/`) **disponibles sólo para captura diagnóstica**, **sin promover ninguna puerta a PASS**; no-objetivos explícitos enumerados (sin aceptación del producto migrado final, sin selección de A / B / C, sin ownership/cutover de FastAPI, sin PASS de paridad de comportamiento G4, sin PASS de ensayo de corte G6); G2 Nivel-2 + G3 Nivel-2 + G4 + G5 + G6 siguen bloqueadas / irreproducibles según auditorías previas; sin tocar código / tests / scripts / fuente; sin commit / push / PR abierto en esta pasada; espejo en inglés actualizado en paralelo)`
