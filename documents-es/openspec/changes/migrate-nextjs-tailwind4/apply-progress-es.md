...
`cutover-manifest.json` canónico sin
                        ...
                        - **2026-09-13** — Pasada de evidencia de cierre delimitada del artefacto canónico de dry-run de G6 rebanada 5 (esta entrada — **pasada de documentación PR 5/6**; `cutover-rehearsal.json` canónico capturado contra el `cutover-manifest.json` canónico en disco; **ningún fichero fuente / tests / scripts / tareas / ficheros de producto / ficheros de evidencia / workspace candidato / `cutover-manifest.json` / `package-lock.json` queda tocado, comiteado, o pusheado en esta pasada**). Según la tarea padre, el artefacto canónico de dry-run de G6 queda capturado en esta rama (`feat/g6-rehearsal-schema-validation-2`, commit `e1fc5a4`) y registrado verbatim aquí y en `design.md::§3.3.6` fila `Disposición (2026-09-13 — artefacto canónico de dry-run de G6 rebanada 5, evidencia de cierre delimitada)` + actualización del pie de `status:`; el espejo en español lleva la traducción fiel. **G6 aprueba para el dry-run canónico**; **G6 NO aprueba la ejecución atomic-cut / ensayo de reversión de Nivel-2** (sigue acoada por el Enfoque A / B / C elegido + PASS de G4 + línea base reproducible de G5); **Nivel-2 de G3, G4, G5 (pasos 2 + 3), y la finalización del cutover NO se afirman en esta pasada.**
                          - **Línea de comando + artefacto** — `python3 scripts/rehearse_cutover.py --manifest openspec/changes/migrate-nextjs-tailwind4/cutover-manifest.json --out /tmp/g6-slice5-dry-run --dry-run` con `cwd = repo-root`; sale `0`; `--dry-run` es REQUERIDO (no se ofrece variante `--execute`). Artefacto canónico emitido atómicamente a `/tmp/g6-slice5-dry-run/cutover-rehearsal.json` (gitignored, reproducible bajo demanda — mismos bytes del manifiesto → mismo hash modulo `validated_at`). Claves del esquema (exactamente 10, congeladas por el pin `EXPECTED_KEY_SET` de la rebanada 4): `consumer_count`, `consumer_ids`, `manifest_path`, `manifest_sha256`, `mode`, `out_dir`, `rollback_executed`, `validated_at`, `validation_errors`, `verification_executed`. NO existe `out_sha256`, NO existe `tier_2_passed`, NO existe `g3_tier2_passed`, NO existe `g4_passed`, NO existe `g5_passed`, NO existe `cutover_complete`, NO existe `rollback_rehearsed` (pin de evidencia delimitada — las claves de Nivel-2 / finalización-del-cutover están explícitamente ausentes).
                          - **Hechos del artefacto canónico** (verbatim, capturado el 2026-09-13): `manifest_path = "openspec/changes/migrate-nextjs-tailwind4/cutover-manifest.json"`; `manifest_sha256 = "3d228ebe5a98081ace5cf088858c0e7fe2b560c77815e5d68e5a01fb375e0c5d"` (coincide con `shasum -a 256` del manifiesto canónico, estable entre ensayos independientes); `validated_at = "2026-09-13T21:47:09.745252+00:00"` (ISO-8601 UTC); `mode = "dry-run"`; `consumer_count = 26`; `consumer_ids[]` = 26 IDs únicos en orden canónico (21 `mount-*` §3.1.1 + 5 `search-urls-*` §3.1.2); `validation_errors = []`; `verification_executed = false`; `rollback_executed = false`.
                          - **Tests que respaldan el PASS** — `python3 -m pytest tests/test_rehearse_cutover.py tests/test_rehearse_cutover_path_text.py -q` → **149 pasaron en ~1.8s** (43 en `tests/test_rehearse_cutover.py` — 31 originales PR 1/6–PR 4/6 + 10 pins canónicos del artefacto de la rebanada 4 + 2 pins nuevos de evidencia delimitada de la rebanada 5: `test_canonical_artifact_bounded_evidence_no_tier2_or_cutover_markers` + `test_canonical_artifact_bounded_evidence_dry_run_only_no_execute_marker`; **106** en `tests/test_rehearse_cutover_path_text.py`), todos en verde en esta rama. Sin skips; sin nuevos fallos.
                          - **Contrato de estabilidad + tests enfocados** — la rama fail-closed de PR #220 junta todos los errores de esquema vía `_validate_manifest`, registra cada error en stderr, sale `EXIT_SCHEMA = 4`, y no emite ningún `cutover-rehearsal.json`; los helpers `parse_shell_text` + `validate_repo_relative_path` de PR #218 descartan rutas absolutas, componentes `..`, caracteres de control / metacaracteres, shell vacío / nulo, comillas sin emparejar en `verification.command` / `rollback` antes de cualquier paso posterior. El `manifest_sha256` del artefacto de ensayo es estable entre corridas consecutivas del verificador (verificado por el pin de la rebanada 4 `test_canonical_artifact_manifest_sha256_stable_across_runs`). Dos tests enfocados nuevos (en la rebanada 5 de PR 5/6) fijan la **intención semántica delimitada** del artefacto canónico que el pin del conjunto de claves de la rebanada 4 por sí solo no puede atrapar (ambos apuntan al manifiesto normalizado canónico vía el helper `_run_canonical` en disco): `test_canonical_artifact_bounded_evidence_no_tier2_or_cutover_markers` (el artefacto NO lleva ninguna de las claves `tier_2_passed`, `g3_tier2_passed`, `g4_passed`, `g5_passed`, `cutover_complete`, `rollback_rehearsed`) + `test_canonical_artifact_bounded_evidence_dry_run_only_no_execute_marker` (conjuntamente `mode == "dry-run"` Y `verification_executed is False` Y `rollback_executed is False` — no se ofreció ninguna variante `--execute`).
                          **Verdad preservada** — G6 **PASS del dry-run canónico registrado el 2026-09-13** (artefacto emitido limpio, 26 / 26 IDs de consumidores de §3.1, `validation_errors` vacío, `verification_executed: false`, `rollback_executed: false`, `mode: "dry-run"`); **la ejecución atomic-cut de Nivel-2 / ensayo de reversión de G6 NO SE EJECUTA** (sigue acoada por el artefacto de build del Enfoque A / B / C elegido + PASS de G4 + línea base reproducible de G5); **Nivel-2 de G3 NO APRUEBA** (sigue bloqueado por G4 + G5 + G6); **G4 NO APRUEBA** (verificador no autordado); **paso 1 del camino de cierre de G5 completo** (según la fila §3.3.5 `Disposición (2026-09-11 — paso 1 del camino de cierre G5)`; pasos 2 + 3 siguen bloqueados); **disposición de G6**: `PASS del dry-run canónico registrado (evidencia de cierre delimitada); ejecución atomic-cut de Nivel-2 / ensayo de reversión TODAVÍA NO SE EJECUTA`; **los Enfoques A / B / C quedan sin seleccionar**; **sin activación de FastAPI** (sin repoint de `WEB_DIR`, sin cutover atómico, sin cambio en `api/server.py` / Makefile / extensión / API / fuente de producto); **`cutover-manifest.json` canónico sin cambios**; el artefacto canónico es la evidencia de la puerta §3.3.6 para el dry-run canónico SOLAMENTE; **no** es un artefacto de Nivel-2 / finalización-del-cutover. Espejo en inglés actualizado en paralelo. Sin commit, push, ni PR abiertos en esta pasada. Total de adiciones autordadas queda bien por debajo del presupuesto de revisión de 400 líneas por PR.
