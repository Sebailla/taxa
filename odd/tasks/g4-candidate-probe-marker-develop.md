...
- [x] ODD-G4-PROBE-DEVELOP-003 Commit the verified work unit when explicitly authorized.
  Route: user-authorized delivery.
  Evidence: `9de87ac fix(g4): add candidate probe marker` contains the layout, focused test, and initial task record only; no pre-existing untracked artifact was staged.
...
- Independent verifier reproduced: focused marker tests `9 passed`; topology guard `2 passed`; `git diff --check` clean.
- User authorized delivery. Committed the verified work unit as `9de87ac fix(g4): add candidate probe marker`; the RDD switch is clone-locally off, and native assessment was unavailable, so the observed writer and independent-verifier evidence remains the verification record.
- Published PR #350 to `develop`: https://github.com/Sebailla/taxa/pull/350. It links `Closes #245`, has exactly `type:bug`, and remains unmerged.
