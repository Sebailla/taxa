...
- [ ] ODD-MIGRATE-002 Define failing React contracts for the research replacement.
  ...
  - Progress: W1 Research domain contract is implemented and independently verified: `src/modules/research/domain/explorer.ts` defines pure, readonly ExplorerState/SearchState/ViewerTab/FileFormat and the legacy-faithful initial-state factory; `tests/test_research_domain.py` passes 20 tests. Independent verification also passed 170 relevant purity/layer tests and strict isolated TypeScript compilation. Work-unit commit pending creation under this authorization; no FastAPI mount change.
