...
## Addendum — 2026-09-08: Canonical composed React capture PASS evidence (isolated React E2E canonical-browser sub-gate only); G4 Playwright + Lighthouse parity / G3 Tier-2 / legacy deletion / cutover unchanged (append-only)

- **Canonical composed React capture PASS (this entry records evidence, not a new landing — isolated React E2E canonical-browser sub-gate only)**. The end-to-end command shape was `make capture-react-e2e OUTPUT_ROOT=<dir>` (equivalently `node tools/react-e2e-harness/scripts/composed-capture.mjs --output-root <dir>`) — no `PLAYWRIGHT_CHROMIUM_EXECUTABLE_PATH` exported, no CLI override flags, harness built fresh (`npm run build` exit `0`, `out/index.html` rendered with `<title>Taxa React E2E Harness — FileExplorer mount</title>`, `data-harness-surface="file-explorer"`, `data-harness-taxon-id="1"`), the in-process fixture API + static-export HTTP server (`5c.2-B.1b-ii-a` + `5c.2-B.1b-ii-b`) bound on OS-assigned loopback ports on `127.0.0.1`, and `chromium-driver.mjs` launched with the pinned `{ headless: true }` options (no `channel`, no `executablePath`). Pin confirmation: `@playwright/test@1.62.1` exactly (canonical Playwright-managed chromium-headless-shell revision `1234` — Chromium 151.0.7922.34), matching the local `playwright-core/browsers.json` + `~/Library/Caches/ms-playwright/chromium_headless_shell-1234/DEPENDENCIES_VALIDATED`. Persisted evidence artifact `<OUTPUT_ROOT>/<ISO-timestamp>/evidence.json` (top-level schema `taxa.react-e2e.run/1` with a nested `taxa.react-e2e.capture/1` payload under `.capture`) reports `browserExecution.canonical = true` (mode `pinned-playwright-default`, `executablePath: null` — no override mark, no diagnostic note), `navigationStatus: 200`, `taxonId: "1"` (non-null), and every React data-contract assertion GREEN (`data-harness-root="react-e2e"`, `data-harness-surface="file-explorer"`, non-null `data-harness-taxon-id="1"`, `[data-explorer="ready"]`, both `[data-pane]` slots, `input[data-search-input]`, ≥1 `[data-file-path]`); the orchestrator's composed `taxa.react-e2e-composed-capture/1` envelope is the in-memory return shape and is emitted to stdout by `composed-capture.mjs` only (not persisted). **Temporary artifact cleanup**: the per-run output directory under `<OUTPUT_ROOT>/<ISO-timestamp>/` (containing `evidence.json`) was verified and removed after evidence capture; no leftover harness `out/`, no stray fixture / export server sockets, no orphan `playwright/.local-browsers` artifacts, no `DEPENDENCIES_VALIDATED` / `INSTALLATION_COMPLETE` marker churn. **Precise limited scope**: this is ONLY the isolated React E2E canonical-browser capture sub-gate — it does NOT close the broader planned G4 Playwright + Lighthouse parity gate (production build vs production API parity + Lighthouse run on the production app still required), G3 Tier-2 (still gated on G4 + G6 closure), `web/*.{html,js,css}` + `tailwind.config.js` legacy deletion, e2e selector modernization on the production component tree, or PR 3e cutover. **G4 / G3 Tier-2 / G6 / cutover remain blocked; no global gate is flipped by this evidence record** — the harness + canonical Playwright pin path is now proven open end-to-end inside the isolated workspace, which unblocks authoring the broader G4 verifier against the production app, but G4 itself stays blocked. Spanish mirror carries the same semantics; any drift is resolved in favour of the English.