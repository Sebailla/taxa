...
- [ ] **5c.2-B remainder (deferred)** — composition slice wiring (fixture + export servers started together by the harness driver), `make capture-react-e2e` target, hermetic capture-driver tests (chromium-driver.mjs wrapped by Python fixtures), e2e selector modernization on the new component tree, `web/*.{html,js,css}` + `tailwind.config.js` legacy deletion. G4 / G3 Tier-2 / cutover remain blocked.
