...
- [ ] **5c.2-B remainder (deferred)** — static export HTTP server orchestration + composition slice, Makefile target, hermetic capture-driver tests, e2e selector modernization on the new component tree, `web/*.{html,js,css}` + `tailwind.config.js` legacy deletion. G4 / G3 Tier-2 / cutover remain blocked.
