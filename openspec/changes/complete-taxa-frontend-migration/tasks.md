...
- [ ] **5c.2-B remainder (deferred)** — fixture API server, export HTTP server orchestration, Makefile target, hermetic driver tests, e2e selector modernization on the new component tree, `web/*.{html,js,css}` + `tailwind.config.js` legacy deletion. G4 / G3 Tier-2 / cutover remain blocked.
