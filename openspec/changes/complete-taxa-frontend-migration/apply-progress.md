...
- **Scope of this attempt (binding)**: allowed edit surfaces
    ...
    ### 2026-09-08 — Canonical composed React capture PASS evidence recorded (isolated React E2E canonical-browser sub-gate only); G4 / G3 Tier-2 / legacy deletion / cutover unchanged

    - **Scope (this entry, evidence only — no new landing)**. The canonical composed React capture was driven end-to-end through the `5c.2-B.1b-ii-c` orchestrator on the harness already shipped (`tools/react-e2e-harness/`). **Evidence command shape**: `make capture-react-e2e OUTPUT_ROOT=<dir>` (equivalently `node tools/react-e2e-harness/scripts/composed-capture.mjs --output-root <dir>`). The invocation ran with NO `PLAYWRIGHT_CHROMIUM_EXECUTABLE_PATH` in the environment and NO CLI override flags; the orchestrator's default `buildFn` ran `npm run build` (exit `0`), verified `out/index.html` access (fail-closed), started the in-process fixture API + static-export HTTP server on OS-assigned loopback ports on `127.0.0.1`, then invoked the default `captureFn` (`run.mjs::capture` → `chromium-driver.mjs::runCapture`) with the pinned `{ headless: true }` launch options (no `channel`, no `executablePath`). **Pin / canonical metadata**: `@playwright/test@1.62.1` exactly → Playwright-managed chromium-headless-shell revision `1234` (Chromium 151.0.7922.34), matching `tools/react-e2e-harness/node_modules/playwright-core/browsers.json` + `~/Library/Caches/ms-playwright/chromium_headless_shell-1234/DEPENDENCIES_VALIDATED`. **Captured envelope** (persisted as `<OUTPUT_ROOT>/<ISO-timestamp>/evidence.json`; top-level schema `taxa.react-e2e.run/1` with a nested `taxa.react-e2e.capture/1` payload under `.capture`): `browserExecution = { canonical: true, mode: "pinned-playwright-default", executablePath: null }` (no override mark, no diagnostic note), `navigationStatus = 200`, `taxonId = "1"` (non-null), all React data-contract assertions GREEN (`data-harness-root="react-e2e"`, `data-harness-surface="file-explorer"`, non-null `data-harness-taxon-id="1"`, `[data-explorer="ready"]`, both `[data-pane]` slots, `input[data-search-input]`, ≥1 `[data-file-path]`); the orchestrator's composed `taxa.react-e2e-composed-capture/1` envelope is the in-memory return shape and is emitted to stdout by `composed-capture.mjs` only (not persisted). **Temporary artifact cleanup**: the per-run output directory under `<OUTPUT_ROOT>/<ISO-timestamp>/` (containing `evidence.json`) was verified and removed after evidence capture; no leftover harness `out/`, no stray fixture / export sockets, no orphan `playwright/.local-browsers` artifacts, no `DEPENDENCIES_VALIDATED` / `INSTALLATION_COMPLETE` marker churn. **Precise limited scope**: this entry records ONLY the isolated React E2E canonical-browser capture sub-gate — it does NOT close the broader planned G4 Playwright + Lighthouse parity gate (production build vs production API parity + Lighthouse run on the production app still required), G3 Tier-2 (still gated on G4 + G6 closure), `web/*.{html,js,css}` + `tailwind.config.js` legacy deletion, e2e selector modernization on the production component tree, or PR 3e cutover. **G4 / G3 Tier-2 / G6 / cutover remain blocked; no global gate is flipped by this evidence record** — the harness + canonical Playwright pin path is now proven open end-to-end inside the isolated workspace, which unblocks authoring the broader G4 verifier against the production app, but G4 itself stays blocked. G1 / G2 / G3 Tier-1 / G5 status rows are also unchanged.
    - **Scope of this attempt (binding)**: edit surfaces limited to the four append-only OpenSpec files (`openspec/changes/complete-taxa-frontend-migration/design.md` + `apply-progress.md` + `documents-es/openspec/changes/complete-taxa-frontend-migration/design-es.md` + `apply-progress-es.md`). No code change; no test change; no lockfile change; no task-list change; no production `src/` change; no `domain/keys.ts` / `infrastructure/store.ts` change; no `tools/react-e2e-harness/**` source change; no production `next.config.mjs` / `package.json` change; no G4 verifier authored; no production build artifact; no FastAPI/SQLite/extension change; no commit/push; no dependency change. All existing uncommitted changes from prior change-log entries (including the version-pin text updates `1.56.0 → 1.62.1` + the chromium-headless-shell revision `1234` annotation across both mirrors) are preserved verbatim. **No gate flip, no cutover authority granted** — G1 / G2 / G3 Tier-1 / G3 Tier-2 / G4 / G5 / G6 status rows and PR 3e cutover-authority rows preserved verbatim from prior change-log entry.
