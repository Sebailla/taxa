...
| Field | Value |
| --- | --- |
| Producer | `scripts/verify_consumers.py` (AUTHORED — merged via PR #109 `test(g3): verify consumer readiness`; PR3d slice augmentation `--serve` / `--venv` / `--repo-root` merged via PR #111 `fix(g3): control readiness verification runtime`) |
