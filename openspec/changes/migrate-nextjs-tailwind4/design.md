...
| G6 | `scripts/rehearse_cutover.py` + `scripts/rehearse_cutover_path_text.py` referencing `design.md::§3.3.6` and the canonical `openspec/changes/migrate-nextjs-tailwind4/cutover-manifest.json` | **PRs 1–4/6 merged on `origin/develop` (PR #217 scaffold + PR #218 path safety + PR #219 manifest normalization + PR #220 fail-closed schema validation); canonical dry-run PASS recorded 2026-09-11 against the normalized 26-consumer manifest (this planning pass is the English-only G6 documentation ledger slice — PR 5/6)** |
...
| Field | Value |
| --- | --- |
| Producer | `scripts/verify_consumers.py` (AUTHORED — verifier merged via PR #109 `test(g3): verify consumer readiness`; controlled-runtime augmentation `--serve` / `--venv` / `--repo-root` / `--fixture-web-root` merged via PR #111 `fix(g3): control readiness verification runtime`; HTTP-shape fail-closed enforcement via `tools/g3-legacy-fixture/scripts/check_http_status.py` merged via PR #115 `fix(g3): enforce HTTP consumer expectations`; virtualenv-symlink preservation merged via PR #116 `fix(g3): preserve virtualenv Python paths`) |
| Command | `python scripts/verify_consumers.py --manifest openspec/changes/migrate-nextjs-tailwind4/cutover-manifest.json --out <build-root> [--serve --venv <repo>/.venv/bin/python --fixture-web-root <repo>/tools/g3-legacy-fixture/web --repo-root <repo>]` |
| Artifact | `<build-root>/CONSUMER-READINESS.json` |
| Threshold | for every consumer listed in §3.1, the canonical `cutover-manifest.json` (§3.3.3.1) names `current_path`, `ownership_edge`, `replacement`, `verification`, `activation_status`, `rollback`; **fail-closed**: any `activation_status: unselected` => verifier exits non-zero and emits no valid `CONSUMER-READINESS.json`; no §3.1 consumer remains "active" against a path that PR3e intends to delete. **Selection is two-tier**: (Tier-1) **legacy pre-cut selection** is authored 2026-08-30 for every consumer against the on-disk legacy `current_path` and **verified on 2026-08-30 by PR #116** for all **26 / 26** consumers via `--serve --venv --fixture-web-root --repo-root` (HTTP-shape fail-closed routing through `tools/g3-legacy-fixture/scripts/check_http_status.py`); (Tier-2) **atomic-cut selection** (post-cut replacement with the chosen Approach A / B / C build artifact) requires G2 + G4 + G5 + G6 PASS and is left for PR3d/PR3e. The Tier-1 / Tier-2 contract rows in §3.3.3.1 (below) make the dual model binding on the future strict-TDD verifier. |
| **Disposition (2026-08-30 canonical evidence — PR #116 merge)** | **PASS — Tier-1 (legacy pre-cut) consumer readiness verified end-to-end on a clean merge of PR #109 + PR #111 + PR #115 + PR #116 onto `origin/develop`.** All **26 / 26** `design.md::§3.1` consumers run their `verification.command` against the controlled fixture (`tools/g3-legacy-fixture/web/` served by `python -m http.server` on an isolated free port via `--fixture-web-root`) with HTTP-shape enforcement routed through `tools/g3-legacy-fixture/scripts/check_http_status.py`, every command exits `0`, and a valid `<build-root>/CONSUMER-READINESS.json` is atomically emitted (schema validates: `manifest_path`, `manifest_sha256`, `node_version ≥ 20.9.0`, `verified_at`, `exit_code = 0`, every `consumers[].status = "ready"`, `unselected_count = 0`, `failed_verifications[]` empty, `activation_complete = true`). **Tier-1 PASS does NOT claim Tier-2 PASS** — Tier-2 (atomic-cut selection against the chosen Approach A / B / C build artifact) remains evidence-gated by G4 + G5 + G6 PASS; Approach A / B / C stay unselected; no FastAPI activation occurs. |
| **Canonical command line (PR #116 evidence capture)** | `python scripts/verify_consumers.py --manifest openspec/changes/migrate-nextjs-tailwind4/cutover-manifest.json --out <build-root> --serve --venv <repo-root>/.venv/bin/python --fixture-web-root <repo-root>/tools/g3-legacy-fixture/web --repo-root <repo-root>` — `--serve` spawns the controlled local server (chosen by `--serve --fixture-web-root` as `python -m http.server` against the fixture's `web/` tree on an OS-picked free TCP port, never the legacy `8765`); `--venv` rewrites any leading `pytest` token in `verification.command` to the resolved venv python (PR #116 preserves the symlink path so Python keeps its venv); `--fixture-web-root` rewrites each consumer's `127.0.0.1:<legacy>` URL to the isolated picked port; HTTP-shape expectations (`"200"`, `"200 for each"`) are routed through `tools/g3-legacy-fixture/scripts/check_http_status.py` (discovered at `<repo-root>/tools/g3-legacy-fixture/scripts/check_http_status.py`; fail-closed if absent — PR #115). Non-HTTP expectations (`"ok"`, `"1 passed"`, `"all passed"`, arbitrary text) keep shell-exit-only semantics. |
| **Coverage (2026-08-30 evidence)** | All **26** `design.md::§3.1` consumers PASS — **21** in §3.1.1 (FastAPI web mount: 2 HTML reads `mount-runtime-html-root-001` + `mount-runtime-html-direct-002`, 1 CSS link `mount-runtime-link-tag-css-003`, 1 JS module-entry `mount-runtime-script-tag-app-js-004`, 4 ES-import `mount-runtime-import-app-js-modules-005` + `mount-runtime-import-nav-js-modules-007` + `mount-runtime-import-breadcrumb-js-008` + `mount-runtime-import-search-js-009`, 3 dynamic-import `mount-runtime-dynamic-import-app-js-settings-006` + `mount-runtime-dynamic-import-nav-js-file-explorer-settings-010` + `mount-runtime-dynamic-import-detail-js-file-explorer-011`, 1 CDN pin `mount-runtime-cdn-pin-file-viewer-012`, 3 smoke/evidence-baseline tests `test-contract-smoke-static-index-013` + `test-contract-smoke-static-app-js-014` + `test-evidence-baseline-legacy-html-015` / `test-evidence-baseline-module-count-016` / `test-evidence-baseline-source-size-017` (folded by coverage summary), 2 build-profile/hydration tests `test-build-profile-emit-keys-018` + `test-hydration-timing-valid-artifact-019`, 1 extension-manifest pin `extension-manifest-origin-pin-020`) + **5** in §3.1.2 (`web/search_urls.js`: 3 detail.js runtime uses `search-urls-runtime-detail-import-001` + `search-urls-runtime-detail-engineByKey-002` + `search-urls-runtime-detail-populate-003`, 2 contract tests `search-urls-test-contract-ac21-004` + `search-urls-test-search-categories-005`). No consumer remains `unselected` against Tier-1. |
| **What this PASS does NOT claim** | (i) It does NOT claim G3 PASS for **Tier-2** (atomic-cut): Tier-2 still requires G4 (Playwright + Lighthouse parity) + G5 (reproducible hydration baseline — currently `unreproducible` per §3.3.5) + G6 (`cutover-rehearsal.json` dry-run success). (ii) It does NOT claim that **Approach A / B / C** is selected; static export (Approach A) and B / C remain unselected until Tier-2 evidence closes. (iii) It does NOT claim **FastAPI activation**: no `WEB_DIR` repoint, no atomic cutover, no `api/server.py` mutation, no Makefile / extension / product-source change is implied. The legacy vanilla frontend continues to be served exactly as on `develop`. (iv) It does NOT alter the canonical `cutover-manifest.json` or any previously emitted `<build-root>/CONSUMER-READINESS.json` — the manifest stays at 26 §3.1 consumers (Tier-1 `selected`, Tier-2 unselected) and the readiness artifact is a **new** emission under the PR #116 evidence capture. |
| **Closure path forward** | (1) Reconstruct the legacy baseline under the current G5 CLI per §3.3.5 closure path so a reproducible candidate-vs-baseline delta exists; (2) author `scripts/rehearse_cutover.py` and produce `cutover-rehearsal.json` per §3.3.6; (3) land the Playwright + Lighthouse parity harness and produce parity reports per §3.3.4 (G4); (4) only after G4 + G5 + G6 close, flip each consumer to Tier-2 `selected` against the chosen Approach's build artifact and re-run the G3 verifier for Tier-2 evaluation; (5) the Tier-2 emission replaces (or augments) the Tier-1 emission with new-path replacements and atomic-cut readiness. Until (1)–(3) close, **G3 stays `passed for Tier-1; not passed for Tier-2`**. |

##### §3.3.3.1 G3 contract definition (canonical input for the strict-TDD G3 verifier)

This subsection is the canonical input for the strict-TDD G3 verifier (`scripts/verify_consumers.py` + `<build-root>/CONSUMER-READINESS.json`; authored via PR #109 + PR #111; HTTP-shape fail-closed enforcement + virtualenv-symlink preservation authored via PR #115 + PR #116). It defines the canonical machine-readable inventory at `openspec/changes/migrate-nextjs-tailwind4/cutover-manifest.json`, the `CONSUMER-READINESS.json` schema, the atomic / failure semantics, and the test fixture requirements. **G3 PASSES for Tier-1 (legacy pre-cut readiness) — the verifier (`scripts/verify_consumers.py`, AUTHORED on disk via PR #109 + PR #111 + PR #115 + PR #116) runs every §3.1 consumer's `verification.command` against the controlled fixture (`tools/g3-legacy-fixture/web/`) with `--serve --venv --fixture-web-root --repo-root`; all 26 / 26 consumers exit `0`; `<build-root>/CONSUMER-READINESS.json` is atomically emitted with `activation_complete: true`, `unselected_count: 0`, `failed_verifications[]` empty, and `exit_code: 0`.** **G3 is NOT PASSED for Tier-2 (atomic-cut selection) — Tier-2 still requires G4 + G5 + G6 PASS; the Tier-2 pass lands in PR3d/PR3e and is NOT in this planning pass.** Selection is **two-tier**: Tier-1 (legacy pre-cut, PASSED 2026-08-30 via PR #116) does not require G2/G4/G5/G6 PASS; Tier-2 (atomic-cut, gated by G4/G5/G6 PASS) lands in PR3d/PR3e and is NOT in this planning pass.

| Knob | Value |
| --- | --- |
| Canonical manifest path | `openspec/changes/migrate-nextjs-tailwind4/cutover-manifest.json` — single source of truth for every active consumer in `design.md::§3.1`. Authored 2026-08-30 with all 26 consumers enumerated (21 in §3.1.1 FastAPI web mount + 5 in §3.1.2 `web/search_urls.js`); **legacy pre-cut selection (Tier-1) authored 2026-08-30 for every consumer** — every consumer carries `activation_status: selected` and `replacement.status: selected` against its on-disk legacy `current_path` (the `replacement.path` field names the legacy file or set of files the consumer currently reads; the `replacement.note` documents the Tier-1 semantics). No consumer remains unselected against the legacy pre-cut baseline; **Tier-1 selection PASSED 2026-08-30 via PR #109 + PR #111 + PR #115 + PR #116 merge** (all 26 / 26 consumers verify clean against the controlled fixture; canonical command + artifact details in §3.3.3 `Disposition` row above). The future atomic-cut pass (Tier-2) replaces (or augments) Tier-1 entries with new-path replacements under G4 + G5 + G6 PASS. |
...
| Consumer record schema | Each entry in `consumers[]` MUST carry the seven fields `id` (stable string), `ownership_edge` (must equal one of `edges[].id`), `current_path` (strict repo-relative path accepted by `validate_repo_relative_path`; the runtime path / test reader path currently active against the chosen ownership edge), `replacement` (`{status: "unselected" | "selected", path?, note?}` where `path` is a strict repo-relative path), `verification`(`{command, expect}`),`activation_status`(`"unselected" | "selected"`),`rollback` (the exact revert statement that restores `current_path`). Human annotations (parenthetical call-outs), brace-expansion lists, comma-separated path enumerations, and line-range facts that previously inflated`current_path` / `replacement.path` MUST move into optional auxiliary fields — `current_paths` (list[str]), `current_path_lines` (str), `current_path_note` (str), `replacement.paths` (list[str]), `replacement.lines` (str), `replacement.path_note` (str) — added only where needed and omitted otherwise; their absence on a record MUST NOT be treated as a missing required field. The verifier rejects any record missing one of the seven required fields and rejects any `current_path` / `replacement.path` that fails `validate_repo_relative_path`. |
| Strict-path rule (PR 3/6) | `current_path` and `replacement.path` MUST each be a single repo-relative slash path (with optional `:line` or `:line:column` suffix) accepted by the merged `validate_repo_relative_path` helper (PR 2/5 — `scripts/rehearse_cutover_path_text.py`). Parentheses, brace-expansion `{a,b,c}.js`, comma-separated path enumerations, line-range spans (e.g. `:77-100`), drive prefixes, shell metacharacters, and `..` components are forbidden in the strict-path fields; the human-readable semantics they encode MUST be carried verbatim by the optional auxiliary fields listed in the `Consumer record schema` row above. PR 4/6 will plug this rule into the strict-TDD G3 verifier via the same helper; PR 3/6 normalizes the canonical manifest on disk. |
...
| Failure semantics (fail-closed, two-tier) | (a) any consumer with `activation_status: unselected` => verifier exits non-zero, emits no `CONSUMER-READINESS.json`; (b) any consumer with `replacement.status: unselected` => same; (c) any consumer missing a required field => same; (d) any consumer with `verification.command` that exits non-zero against the legacy pre-cut runtime (Tier-1 evaluation) OR against `<build-root>` (Tier-2 evaluation) => same; (e) Tier-2 evaluation may not start unless Tier-1 evaluation passes AND G2/G4/G5/G6 PASS, and a Tier-2 fail closes the artifact the same way as a Tier-1 fail; (f) Node version below `20.9.0` => same (parity with G2); (g) `CONSUMER-READINESS.json` is written **only** atomically (temp file + rename) and **only** when every precondition above holds. **No silent fallback to legacy files is permitted under any branch.** |
...
| Selection rule — Tier-1 (legacy pre-cut) | To flip a consumer to `selected` under Tier-1 (legacy pre-cut, recorded 2026-08-30 for every consumer), the manifest MUST name `replacement.path` equal to (or consistent with) the on-disk legacy `current_path` AND `replacement.note` MUST document the Tier-1 semantics AND `verification.command` MUST point at the legacy runtime reachable on FastAPI's 127.0.0.1:8765 mount (or the equivalent file-reader path). **Tier-1 does NOT require G2/G4/G5/G6 PASS** — Tier-1 is a documentation artifact that names the legacy pre-cut destination per consumer; the legacy vanilla frontend continues to be served exactly as on `develop` (no `WEB_DIR` repoint, no atomic cutover, no FastAPI activation). **Tier-1 IS a G3 PASS** — every Tier-1 consumer was verified 2026-08-30 via PR #109 + PR #111 + PR #115 + PR #116 merge against the controlled fixture with `--serve --venv --fixture-web-root --repo-root`; all 26 / 26 consumers exit `0`; `<build-root>/CONSUMER-READINESS.json` is atomically emitted (see §3.3.3 `Disposition` row above for the canonical PASS evidence). |
| Selection rule — Tier-2 (atomic-cut, gated) | To flip a consumer to `selected` under Tier-2 (atomic-cut, post-cut replacement with the chosen Approach A / B / C build artifact), the manifest MUST name `replacement.path` + `verification.command` + `verification.expect` for the **new** path AND failing-test evidence for the new path MUST exist AND G2 (`BUILD-INVENTORY.json` reproducible — already `passed` 2026-08-30 from `taxa-worktrees/migrate-nextjs-g2-evidence-capture`) + G4 (Playwright + Lighthouse parity) + G5 (reproducible hydration baseline — currently `unreproducible` per §3.3.5 audit) + G6 (`cutover-rehearsal.json` dry-run success) MUST all be `passed`. Static export (Approach A) and any other Approach (B / C) remain unselected until those gates close; **no consumer is flipped to Tier-2 in this planning pass**. Tier-2 entry recorded alongside (NOT replacing) Tier-1 entries when PR3d/PR3e runs. |
| Combined selection state | The 2026-08-30 manifest state is **Tier-1 selected for every consumer, Tier-2 unselected for every consumer**. The verifier (authored via PR #109 + PR #111 + PR #115 + PR #116; on-disk at `scripts/verify_consumers.py` + tests at `tests/test_verify_consumers.py`) evaluates Tier-1 first (legacy pre-cut readiness — each `verification.command` exits 0 against the legacy runtime via the controlled fixture + `--serve --venv --fixture-web-root --repo-root`) and Tier-2 second (atomic-cut readiness — each `verification.command` exits 0 against `<build-root>` AND G2/G4/G5/G6 PASS). **Tier-1 evaluation passed 2026-08-30 (PASS — all 26 / 26 consumers exit 0; `CONSUMER-READINESS.json` emitted with `activation_complete: true`, `unselected_count: 0`, `failed_verifications[]` empty, `exit_code: 0`).** Tier-2 evaluation is not yet run; a Tier-2 fail-closed (when Tier-2 is run) fires only after Tier-1 passes, and Tier-2 requires G4 + G5 + G6 PASS to succeed. |
| Provenance (this planning pass) | Manifest authored 2026-08-30 from `design.md::§3.1` (active-consumer inventory) verbatim; every consumer ID was issued from a fresh `mount-` or `search-urls-` namespace; no consumer was collapsed or merged. **2026-08-30 legacy pre-cut selection pass (this section, second update):** every consumer's `replacement.path`, `replacement.status`, and `activation_status` are flipped to `selected` against the on-disk legacy `current_path`; the `selection_invariants` block carries a two-tier selection model (Tier-1 legacy pre-cut, Tier-2 atomic-cut gated by G2/G4/G5/G6); `all_replacements_unselected` flips to `false`; `fail_closed_summary` and `verifier_contract_summary` are updated with the Tier-1 / Tier-2 threshold language. **2026-08-30 Tier-1 PASS record pass (this section, third update — PR #116 merge):** the canonical G3 PASS evidence is captured via `python scripts/verify_consumers.py --manifest openspec/changes/migrate-nextjs-tailwind4/cutover-manifest.json --out <build-root> --serve --venv <repo-root>/.venv/bin/python --fixture-web-root <repo-root>/tools/g3-legacy-fixture/web --repo-root <repo-root>` (all four PR #109 / #111 / #115 / #116 features active: verifier exists, controlled runtime exists, HTTP-shape enforcement exists, venv-symlink preservation exists); every §3.1 consumer's `verification.command` runs against the controlled fixture with HTTP-shape fail-closed routing through `tools/g3-legacy-fixture/scripts/check_http_status.py`; all 26 / 26 exit `0`; `<build-root>/CONSUMER-READINESS.json` is atomically emitted with `activation_complete: true`, `unselected_count: 0`, `failed_verifications[]` empty, `exit_code: 0`. **Tier-1 PASS is recorded; Tier-2 atomic-cut selection remains evidence-gated by G4 + G5 + G6 PASS; no FastAPI activation is implied; Approach A / B / C stay unselected.** The Spanish mirror `documents-es/openspec/changes/migrate-nextjs-tailwind4/design-es.md::§3.3.3.1` carries the faithful Spanish translation of this row. |
...
| **Disposition (2026-09-11 — G5 closure-path step 1)** | **Step 1 complete — legacy baseline capture is now reproducible under the documented G5 CLI; comparison still requires steps 2 + 3.** `scripts/measure_hydration.py` was extended with a hermetic `--baseline --candidate --iterations` capture mode that walks the candidate root for `*.html / *.css / *.js` source bytes and writes a deterministic baseline artifact; the original PR 1b.3a positional `validate <artifact>` contract is preserved verbatim (exit codes 0/1/2/3 — the `argparse` rewrite maps usage errors back to exit 1 so the original "no-argument → exit 1" contract holds). The capture is **hermetic** (no live browser, no network, no live FastAPI server) so the same candidate root produces byte-identical artifacts (modulo wall-clock `captured_at`, which the regression test normalises out). The closure-path (1) missing-proof inventory is closed for the baseline side: (i) capture command line is recorded in `provenance.command_line`; (iv) iteration count is recorded in `provenance.iterations`; (v) + (vi) raw Playwright / Lighthouse JSON is **not** emitted (out of scope for the hermetic capture); (viii) the existing 6-key schema (`captured_at`, `build`, `route`, `server_shell`, `client_render`, `console_warnings`) is preserved verbatim so the legacy `validate <artifact>` mode covers the baseline without modification. **Test coverage** — 6 new focused tests in `tests/test_hydration_timing.py` (`capture_writes_baseline_artifact`, `capture_is_byte_reproducible`, `capture_fails_on_missing_candidate`, `capture_validates_written_artifact`, `capture_requires_candidate`, `exits_one_on_no_arguments`) all green; the 11 original tests still green. **Truth preserved** — G5 closure-path step (2) (candidate-side capture against the chosen Approach's build artifact) remains blocked by G2 + G4; G5 closure-path step (3) (joining baseline + candidate) remains blocked by step (2); G5 closure-path step (4) (asserting ±10 % thresholds) remains blocked by step (3); the legacy-baseline location `docs/baselines/legacy-web-2026-08-26.json` is **gitignored** by `.gitignore` (`docs/baselines/`); no pre-existing artifact ships in the repo — the baseline is reproducible on demand (same input → same bytes, modulo `captured_at`) so a clean clone can regenerate it by running `scripts/measure_hydration.py --baseline docs/baselines/legacy-web-2026-08-26.json --candidate <legacy-server-root> --iterations 10` against `tools/g3-legacy-fixture/web/` without contacting any external system; no FastAPI activation (no `WEB_DIR` repoint, no atomic cutover); Approach A / B / C remain unselected; G3 Tier-1 still PASS, Tier-2 still blocked by G4 + G5 + G6; G4 still blocked; **G6 PASS recorded 2026-09-11 (canonical dry-run over normalized 26-consumer manifest; PR #217 + #218 + #219 + #220 merged on `origin/develop`)**, Tier-2 atomic-cut execution / rollback-rehearsal still evidence-gated; the legacy baseline artifact is NOT a substitute for a real Playwright capture — it is the §3.3.5 closure-path (1) reproducible shape on disk; the real Playwright capture replaces the synthetic numbers with measured ones in PR3d and leaves this artifact behind for reproducibility. |
| **Reproducibility contract (2026-09-11)** | **The G5 closure-path step 1 capture is byte-deterministic** for a given candidate root. Every value except `captured_at` (and the recorded `provenance.command_line` which legitimately reflects the user-provided argv) is reproducible: same candidate root + same `--iterations` → same artifact (modulo the wall-clock timestamp). The contract is pinned by `tests/test_hydration_timing.py::test_measure_hydration_capture_is_byte_reproducible` which runs the capture twice against the same root and asserts byte equality after normalising `captured_at` + `provenance.command_line` out of the comparison. A future regression that introduces non-determinism (random iteration order, `time.time()` in the artifact body, unordered dict keys, etc.) is captured by this test. |
| **Bridge proof logs (Slice 4 of the G5 orchestrator CLI)** | **Authored.** The Python orchestrator (`scripts/run_g5_orchestration.py`) owns bridge proof logs; the Node bridge (`tools/g4-capture/scripts/g5_raw_lhr_bridge.mjs`) remains side-effect free. New CLI flag `--bridge-log-dir` defaults to `<out>/raw/bridge-logs`; on every bridge invocation the CLI atomically persists `bridge-NN.json` (NN = two-digit iteration) carrying `schema=taxa.g5-bridge.proof-log/1`, `argv`, `cwd`, `timestamp` (ISO-8601 UTC), `timeout_s`, `duration_ms`, `exit_code`, `timed_out`, `stdout`, and `stderr`. Previous-run logs are rotated on first write (each new file produced via temp-file + `os.replace`). Log-write failure is non-blocking — a distinct stderr advisory `[run_g5_orchestration] bridge-log write failed: ...` is emitted and the bridge success / sentinel / BridgeError contract is preserved. Hermetic tests cover success, non-zero exit, timeout, write failure, filename / rotation, and CLI threading / validation / dry-run isolation. **Resolves missing-proof items (i) capture command line (argv), (ii) capture stdout/stderr log, (iv) iteration count, and (vi) raw Lighthouse JSON wrapper** by binding every per-iteration bridge invocation to a JSON proof log on disk under `<out>/raw/bridge-logs/`. Still missing for full G5 reproducibility: (iii) capture environment (Node version, chromium SHA256 pin, FastAPI version, FastAPI route hash), (v) raw Playwright JSON, (vii) candidate-vs-baseline delta row, (viii) match to legacy baseline schema. |

#### §3.3.6 G6 — cutover rehearsal

| Field | Value |
| --- | --- |
| Producer | `scripts/rehearse_cutover.py` + `scripts/rehearse_cutover_path_text.py` (PRs 1–4/6 merged on `origin/develop`: PR #217 scaffold + dry-run-only CLI; PR #218 path / shell-text validators; PR #219 canonical manifest path normalization; PR #220 fail-closed schema validation delegating path + shell rules to the PR #218 helpers — no duplicated logic) |
| Command | `python scripts/rehearse_cutover.py --manifest openspec/changes/migrate-nextjs-tailwind4/cutover-manifest.json --out <out> --dry-run` (`--dry-run` is REQUIRED — no `--execute` variant is offered; `verification.command` and `rollback` are NEVER invoked) |
| Artifact | `<out>/cutover-rehearsal.json` (PR #217 emits a minimal artifact with `manifest_path`, `manifest_sha256`, `validated_at`, `mode: "dry-run"`, `out_dir`, `consumer_count`, `verification_executed: false`, `rollback_executed: false`; PR #220 extends with `consumer_ids[]` (one per consumer, declaration order) + `validation_errors[]` (empty on success) after the fail-closed schema branch) |
| Threshold | on success: `cutover-rehearsal.json` is atomically emitted under `<out>/` via temp-file + `os.replace`; `consumer_count` equals the canonical manifest's `consumers[]` length; every consumer in declaration order is named in `consumer_ids[]` with `id`, `current_path`, `replacement.path`, `verification.{command, expect}`, `rollback` passing `validate_repo_relative_path` + `parse_shell_text` (PR #218 helpers); `validation_errors` is `[]`; `verification_executed` is `false`; `rollback_executed` is `false`; `manifest_sha256` is stable across consecutive verifier runs. Fail-closed branches: missing / non-list `consumers` ⇒ `EXIT_SCHEMA = 4`, no artifact; any consumer missing a required field ⇒ `EXIT_SCHEMA = 4`, no artifact; duplicate `id` ⇒ `EXIT_SCHEMA = 4`, no artifact; any `current_path` / `replacement.path` failing `validate_repo_relative_path` ⇒ `EXIT_SCHEMA = 4`, no artifact; any `verification.command` / `rollback` failing `parse_shell_text` ⇒ `EXIT_SCHEMA = 4`, no artifact; `--dry-run` omitted ⇒ `EXIT_USAGE = 2`, no artifact. **G6 is `PASSED for canonical dry-run; Tier-2 atomic-cut execution / rollback-rehearsal still evidence-gated`** — the rehearsal script is dry-run-only by design (no `--execute` variant is offered); atomic-cut selection remains evidence-gated by G2 PASS + G4 + G5 + G6 (here: G6 == dry-run PASS) and is NOT in this slice. |
| **Disposition (2026-09-11 — canonical G6 dry-run PASS)** | **PASS — canonical dry-run over the normalized 26-consumer `openspec/changes/migrate-nextjs-tailwind4/cutover-manifest.json` exits `0` and atomically emits a complete `cutover-rehearsal.json` artifact under `<out>/`, with `consumer_count = 26`, 26 unique `consumer_ids[]` in declaration order, `validation_errors = []`, `verification_executed = false`, `rollback_executed = false`, `mode = "dry-run"`, `validated_at` ISO-8601 UTC, `manifest_path` matching the canonical machine-readable manifest, and `manifest_sha256` matching the on-disk canonical manifest hash (stable across consecutive verifier runs).** The merged scaffold — **PR #217 (PR 1/6)** `scripts/rehearse_cutover.py` + `tests/test_rehearse_cutover.py` (dry-run-only CLI, refuses without `--dry-run`, atomic temp-file + `os.replace` write, exit codes `EXIT_OK = 0` / `EXIT_USAGE = 2` / `EXIT_SCHEMA = 4` / `EXIT_IO = 5`); **PR #218 (PR 2/6)** `scripts/rehearse_cutover_path_text.py` + `tests/test_rehearse_cutover_path_text.py` (pure-stdlib `validate_repo_relative_path` + `parse_shell_text` helpers); **PR #219 (PR 3/6)** canonical `cutover-manifest.json` path normalization + path-safety triangulation tests; **PR #220 (PR 4/6)** `scripts/rehearse_cutover.py` fail-closed schema validation (delegates path + shell rules to the PR #218 helpers, collects all errors via `_validate_manifest`, logs every problem to stderr, exits `EXIT_SCHEMA = 4`, emits no artifact) — is on disk at `origin/develop` HEAD `dd1c218` and the canonical dry-run executes against that HEAD without test fixtures. **Pass tests** — `python3 -m pytest tests/test_rehearse_cutover.py tests/test_rehearse_cutover_path_text.py -q` → **137 passed in 1.40s** (31 in `tests/test_rehearse_cutover.py` + 106 in `tests/test_rehearse_cutover_path_text.py`); the canonical happy-path pin `tests/test_rehearse_cutover.py::test_canonical_happy_path_emits_artifact_for_all_26_consumers` enforces `r.returncode == 0`, `(out / "cutover-rehearsal.json").is_file()`, 26 unique `consumer_ids`, empty `validation_errors`, `consumer_count == 26`, `verification_executed is False`, `rollback_executed is False`. **Canonical command line** — `python3 scripts/rehearse_cutover.py --manifest openspec/changes/migrate-nextjs-tailwind4/cutover-manifest.json --out <out> --dry-run` with `cwd = repo-root`; exit `0` against the canonical normalized manifest. **Stability** — the PR #220 fail-closed schema branch collects all schema errors via `_validate_manifest`, logs every error to stderr, exits `EXIT_SCHEMA = 4`, and emits no `cutover-rehearsal.json`; the PR #218 `parse_shell_text` + `validate_repo_relative_path` helpers rule out absolute paths, `..` components, control chars / metachars, empty / null-shell, unmatched-quote `verification.command` / `rollback` strings before any further step. **Risk note** — the canonical dry-run PASS is a **planning-only** evidence record for the §3.3.6 gate; **G6 remains dry-run-only by design** (no `--execute` variant is offered — `verification.command` / `rollback` are never invoked; rollback rehearsal is therefore still **planned, not exercised**, and atomic-cut selection remains evidence-gated by G2 PASS + G4 PASS + G5 PASS, none of which constitute `verification.command` execution against a chosen Approach's build artifact). **Truth preserved** — G2 PASS recorded 2026-08-30 (unchanged); G3 **PASSED** for Tier-1, **NOT PASSED** for Tier-2 (Tier-2 atomic-cut is still evidence-gated by G4 + G5 + G6, and atomic-cut requires a chosen Approach A / B / C, none of which is selected); G4 still `blocked` (verifier not authored); G5 closure-path **step 1 complete**, steps 2 + 3 still blocked; Approach A / B / C remain unselected; no FastAPI activation (no `WEB_DIR` repoint, no atomic cutover, no `api/server.py` / Makefile / extension / API / product-source change); canonical `cutover-manifest.json` is unchanged (26 §3.1 consumers, Tier-1 `selected`, Tier-2 unselected); the previously emitted `CONSUMER-READINESS.json` (if any) is unchanged. **G6 disposition: `PASSED for canonical dry-run; Tier-2 atomic-cut execution / rollback-rehearsal still evidence-gated`**; no source / tests / scripts / config / Makefile / extension / API / product-source touched, committed, or pushed in this pass; English-only ledger slice — Spanish docs (and the Spanish mirror `documents-es/openspec/changes/migrate-nextjs-tailwind4/design-es.md`) stay unchanged.
| **Disposition (2026-09-13 — G6 slice 5 canonical dry-run artifact, bounded closure evidence)** | **Canonical dry-run PASS recorded (bounded closure evidence); Tier-2 atomic-cut execution / rollback-rehearsal NOT YET EXECUTED.** Per the parent task, the canonical `cutover-rehearsal.json` is captured on this branch (`docs/g6-bounded-closure`) at the canonical-artifact-pin commit `66a0015` (the merge of #220's slice-4 work, functionally equivalent to the source branch `feat/g6-rehearsal-schema-validation-2`'s `e1fc5a4`) via `python3 scripts/rehearse_cutover.py --manifest openspec/changes/migrate-nextjs-tailwind4/cutover-manifest.json --out /tmp/g6-bounded-closure-docs-dry-run --dry-run` with `cwd = repo-root`; exit `0`; `--dry-run` is REQUIRED (no `--execute` variant is offered). The canonical artifact is atomically written to `/tmp/g6-bounded-closure-docs-dry-run/cutover-rehearsal.json` (gitignored, reproducible on demand — same canonical manifest bytes → same artifact hash modulo wall-clock `validated_at`). Schema keys (canonical, exactly 10, frozen by the slice-4 `EXPECTED_KEY_SET` pin): `consumer_count`, `consumer_ids`, `manifest_path`, `manifest_sha256`, `mode`, `out_dir`, `rollback_executed`, `validated_at`, `validation_errors`, `verification_executed`. NO `out_sha256`, NO `tier_2_passed`, NO `g3_tier2_passed`, NO `g4_passed`, NO `g5_passed`, NO `cutover_complete`, NO `rollback_rehearsed`, NO `atomic_cut_executed`, NO `fastapi_activated`, NO `web_dir_repointed` keys exist (bounded-evidence pin — the artifact is the script's self-description for the canonical dry-run ONLY; Tier-2 / cutover-completion keys are explicitly absent so the artifact cannot be misread as Tier-2 / cutover closure). **Canonical artifact facts** (verbatim, captured 2026-09-17 in the canonical dry-run adapted from PR #258): `manifest_path = "openspec/changes/migrate-nextjs-tailwind4/cutover-manifest.json"`; `manifest_sha256 = "3d228ebe5a98081ace5cf088858c0e7fe2b560c77815e5d68e5a01fb375e0c5d"` (matches `shasum -a 256` of the canonical manifest on disk, stable across independent rehearsals); `validated_at = "2026-09-17T08:51:44.152763+00:00"` (ISO-8601 UTC); `mode = "dry-run"`; `consumer_count = 26`; `consumer_ids[]` = 26 unique consumer IDs in canonical declaration order (21 `mount-*` §3.1.1 + 5 `search-urls-*` §3.1.2); `validation_errors = []`; `verification_executed = false`; `rollback_executed = false`. **Tests supporting the PASS** — `python3 -m pytest tests/test_rehearse_cutover.py tests/test_rehearse_cutover_path_text.py -q` → **149 passed in ~1.86s** (43 in `tests/test_rehearse_cutover.py` — 31 original PR 1/6–PR 4/6 tests + 10 slice-4 canonical artifact pins + **2 new slice-5 bounded-evidence pins**: `test_canonical_artifact_bounded_evidence_no_tier2_or_cutover_markers` + `test_canonical_artifact_bounded_evidence_dry_run_only_no_execute_marker`; **106** in `tests/test_rehearse_cutover_path_text.py`), all green on this branch. No skips; no new failures. **What this PASS does NOT claim** — (i) It does NOT claim G6 Tier-2 atomic-cut execution / rollback-rehearsal PASS (Tier-2 still evidence-gated by chosen Approach A / B / C build artifact + G4 PASS + G5 reproducible baseline). (ii) It does NOT claim G3 Tier-2 PASS, G4 PASS, or G5 closure-path steps 2 + 3 PASS. (iii) It does NOT claim cutover completion — no `WEB_DIR` repoint, no atomic cutover, no `api/server.py` / Makefile / extension / API / product-source change. (iv) It does NOT alter the canonical `cutover-manifest.json`; the canonical `cutover-rehearsal.json` is **not** committed to the repo (lives under `/tmp`, gitignored, reproducible on demand). The artifact is the §3.3.6 gate evidence for canonical dry-run ONLY; it is **not** a Tier-2 / cutover-completion artifact. |
| **Closure path forward** | (1) Author the G6 `--execute` surface + Tier-2 rollback rehearsal once Approach A / B / C is selected AND G4 PASSES AND G5 closure-path steps 2 + 3 close (the canonical dry-run artifact at `/tmp/g6-bounded-closure-docs-dry-run/cutover-rehearsal.json` is the §3.3.6 gate evidence for canonical dry-run ONLY; a Tier-2 emission replaces (or augments) the canonical dry-run artifact with atomic-cut readiness, rollback-rehearsal evidence, and an `out_sha256` of the served artifact root); (2) land the Playwright + Lighthouse parity harness and produce parity reports per §3.3.4 (G4); (3) only after G4 + G5 closure-path steps 2 + 3 + G6 Tier-2 close, flip each consumer to Tier-2 `selected` against the chosen Approach's build artifact and re-run the G3 verifier for Tier-2 evaluation; (4) the Tier-2 emission replaces (or augments) the Tier-1 emission with new-path replacements and atomic-cut readiness. Until (1)–(3) close, **G6 stays `canonical dry-run PASS recorded (bounded closure evidence); Tier-2 atomic-cut execution / rollback-rehearsal NOT YET EXECUTED`**. |
 |

---

`status: complete (PR 2a slice; PR3a boundary scope planning with G2–G6 evidence producer/threshold plan appended — no boundary selected for Approach A / B / C; G2 contract defined in §3.3.2.1 with **four** explicit maintainer corrections applied against the verified Next.js 16.3.3 / Turbopack build output contract: (1)`size:exception` ONLY for `tools/g2-candidate/package-lock.json`, only after`npm ci` exits 0 against the candidate's local `package.json`, no other generated file excepted; (2) atomic post-build manifest staging from`<candidate-root>/.next/` → `<candidate-root>/out/.next/` — `build-manifest.json` required (its absence is a missing-class failure), `app-build-manifest.json` optional and never a missing-class failure (best-effort copy recorded as `staged` / `not_emitted`); (3)`index.html` sole normal application-route HTML entry, `404.html`/`500.html` explicitly permitted error-page exemptions classified under a separate `error_pages` asset class; (4) **Next 16 / Turbopack output-contract correction** — required **CSS** class = one-or-more non-empty `*.css` under `out/_next/static/chunks/**` (not `out/_next/static/css/`); required **JS** class = one-or-more non-empty`*.js` under `out/_next/static/chunks/**` (no `chunks/app/` subdirectory requirement); G2 PASS recorded 2026-08-30 from clean worktree `taxa-worktrees/migrate-nextjs-g2-evidence-capture` off `develop@a74289b` (build finished `2026-08-30T18:11:02Z`, Node`v26.8.1`, inventory at`taxa-worktrees/migrate-nextjs-g2-evidence-capture/tools/g2-candidate/out/BUILD-INVENTORY.json` with `missing_classes[]` empty, all required application-route classes present, `build-manifest.json` staged at 607 bytes / sha256 `f52f7edd901e373a2a24a4ecf8ba61c96ad227093c6440dc4a3a6ca58a92f2a3`, optional`app-build-manifest.json` `not_emitted`, 14 verifier + 34 candidate focused tests pass, build log captured — multi-lockfile warning non-blocking); G5 legacy baseline dispositioned unreproducible per §3.3.5 2026-08-30 audit; G3 contract defined in §3.3.3.1 with canonical machine-readable manifest at`openspec/changes/migrate-nextjs-tailwind4/cutover-manifest.json` authored 2026-08-30 enumerating all 26 §3.1 consumers (21 §3.1.1 FastAPI web mount + 5 §3.1.2 web/search_urls.js) with stable IDs, `current_path`,`ownership_edge`,`replacement`,`verification`,`activation_status`,`rollback`; **legacy pre-cut selection (Tier-1) authored 2026-08-30 for every consumer** — every consumer carries`activation_status: selected` and `replacement.status: selected` against its on-disk legacy `current_path`;`all_replacements_unselected` flipped to `false`; **G3 Tier-1 PASS recorded 2026-08-30 via PR #109 + PR #111 + PR #115 + PR #116 merge** — canonical command`python scripts/verify_consumers.py --manifest openspec/changes/migrate-nextjs-tailwind4/cutover-manifest.json --out <build-root> --serve --venv <repo-root>/.venv/bin/python --fixture-web-root <repo-root>/tools/g3-legacy-fixture/web --repo-root <repo-root>`; all **26 / 26** §3.1 consumers run`verification.command`against the controlled fixture (`tools/g3-legacy-fixture/web/` served by `python -m http.server` on an isolated free port) with HTTP-shape fail-closed enforcement via `tools/g3-legacy-fixture/scripts/check_http_status.py`; each exits`0`;`<build-root>/CONSUMER-READINESS.json` atomically emitted with `manifest_path`,`manifest_sha256`,`node_version ≥ 20.9.0`,`verified_at`,`exit_code = 0`, every`consumers[].status = "ready"`,`unselected_count = 0`,`failed_verifications[]` empty, `activation_complete = true`; **G3 PASSES for Tier-1 (legacy pre-cut readiness)**; **G3 does NOT pass for Tier-2** — Tier-2 (atomic-cut selection against the chosen Approach A / B / C build artifact) remains evidence-gated by G4 + G5 + G6 PASS; Approach A / B / C remain unselected; Tier-2 atomic-cut pass lands in PR3d/PR3e after G4 / G5 / G6 close; the §3.3.3.1 contract defines a **two-tier selection model** — Tier-1 (PASSED 2026-08-30, legacy pre-cut, no G2/G4/G5/G6 PASS required) AND Tier-2 (atomic-cut, gated by G4/G5/G6 PASS, NOT in this pass);`fail_closed_summary` and `verifier_contract_summary`updated with Tier-1 / Tier-2 thresholds; **fail-closed (Tier-1 + Tier-2)**: G3 verifier (`scripts/verify_consumers.py`, AUTHORED via PR #109 + PR #111 + PR #115 + PR #116; on-disk at`scripts/verify_consumers.py` + tests at `tests/test_verify_consumers.py`) emits no valid`CONSUMER-READINESS.json` while any `verification.command` exits non-zero OR while any G4/G5/G6 gate fails (Tier-2) OR while any required field is missing OR while Node version is below `20.9.0` OR while HTTP-shape expects lack the `check_http_status.py` helper (PR #115 fail-closed); G4 still blocked (verifier not authored); **G6 PASS recorded 2026-09-11 (this planning pass — English-only ledger slice — PR 5/6)** — canonical dry-run over the normalized 26-consumer `openspec/changes/migrate-nextjs-tailwind4/cutover-manifest.json` exits `0` and atomically emits a complete `cutover-rehearsal.json`artifact (`consumer_count = 26`, 26 unique`consumer_ids[]`,`validation_errors = []`,`verification_executed = false`,`rollback_executed = false`,`mode = "dry-run"`); 137 focused tests pass (`tests/test_rehearse_cutover.py` 31 / `tests/test_rehearse_cutover_path_text.py` 106); the merged scaffold is PR #217 + #218 + #219 + #220 (scaffold + path safety + manifest normalization + fail-closed schema validation) on `origin/develop` HEAD `dd1c218`— G6 is **`PASSED for canonical dry-run`**; **Tier-2 atomic-cut execution / rollback-rehearsal still evidence-gated** (no`--execute` variant is offered by design; `verification.command` / `rollback` are never invoked; the rehearsal artifact is a planning-only emission against the canonical dry-run contract, not an activation evidence); no source / tests / scripts / config / Makefile / extension / API / product-source touched, committed, or pushed in this pass; G5 **closure-path step 1 complete** (legacy baseline capture reproducible under the documented G5 CLI per §3.3.5 `Disposition (2026-09-11 — G5 closure-path step 1)` row; `scripts/measure_hydration.py` extended with hermetic capture mode only; compare mode DEFERRED to step 3; `docs/baselines/` gitignored; 6 focused tests green); G5 **closure-path steps 2 + 3 still blocked** (candidate-side capture requires G2 + G4 evidence; joining baseline + candidate through the G5 command requires step 2; asserting ±10 % thresholds requires step 3); G5 **disposition**: `closure-path step 1 complete; comparison not yet attempted (steps 2 + 3 pending)`; static export unselected (Approach A / B / C remain unselected); no FastAPI activation (no`WEB_DIR` repoint, no atomic cutover, no `api/server.py` / Makefile / extension / API / product-source change); **G6 disposition: `canonical dry-run PASS recorded 2026-09-13 (bounded closure evidence); Tier-2 atomic-cut execution / rollback-rehearsal NOT YET EXECUTED`** (canonical artifact captured at`/tmp/g6-bounded-closure-docs-dry-run/cutover-rehearsal.json` per §3.3.6 `Disposition (2026-09-13 — G6 slice 5 canonical dry-run artifact, bounded closure evidence)` row; 26 / 26 §3.1 consumer IDs enumerated, empty `validation_errors`,`verification_executed: false`,`rollback_executed: false`,`mode: "dry-run"`; **G3 disposition:`PASSED for Tier-1; NOT PASSED for Tier-2`**; Tier-2 atomic-cut pass lands in PR3d/PR3e after G4 / G5 / G6 close; no boundary selected beyond G2 PASS; no source / tests / scripts / config / Makefile / extension / API / product-source touched, committed, or pushed in this pass; Spanish mirror updated in lockstep)`
