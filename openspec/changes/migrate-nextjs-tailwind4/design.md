...
| Threshold | server-shell first-paint within ±10 % of legacy baseline; interaction latency (key tabs / search dropdown) within ±10 % of legacy baseline; bundle size within declared threshold (candidate total uncompressed JS + CSS ≤ baseline total × 1.10; full definition in Bundle gate row below) |
...
| **Provisional G2 candidate (existing, diagnostic only)** | The bundle-gate candidate is the existing `tools/g2-candidate/` G2 diagnostic shell (authorized non-activation build root per §3.3.2.1 row 1). It is **provisional infrastructure evidence only** — it exists for diagnostic capture, **NOT** as a selection of Approach A / B / C and **NOT** as a candidate for FastAPI activation. The candidate's emitted build output (`<candidate-root>/out/`) is read-only for measurement; mounting it under FastAPI's `127.0.0.1:8765` StaticFiles remains a separate G3+G6 decision (blocked). |
| **Bundle gate (provisional, planning-only)** | The previously-TBD bundle-size line of the G5 Threshold row is now defined (provisional, planning-only) as: **candidate total uncompressed emitted JS + CSS ≤ legacy baseline total uncompressed JS + CSS × 1.10** (≤ 10 % delta on the combined uncompressed bundle weight). Candidate JS/CSS weight = sum of bytes across every non-empty `*.js` file under `<candidate-root>/out/_next/static/chunks/**` + every non-empty `*.css` file under the same path (co-located per §3.3.2.1 `Required asset classes (application-route)` + the **four**-correction contract). Baseline = legacy `web/` JS/CSS total from the reproducible legacy capture (closure-path step 1). The 10 % delta is a **provisional planning gate**; it does **NOT** constitute Approach selection, FastAPI activation, or migrated-product acceptance. |
| **Capture infrastructure availability (existing, provisional)** | Existing on disk for **diagnostic / capture-only use** (provisional, **NOT** promoting any gate to PASS): (a) `tools/g4-capture/` (G4/G5 capture tree landed via PRs #131–#140 on `docs/g5-provisional-candidate` at `3cee69b` — controlled hydration fixture, capture preconditions, raw Playwright samples, raw Lighthouse payloads, atomic publication, readiness latency, raw→published bridge, controlled legacy lifecycle, capture orchestration); (b) `tools/g3-legacy-fixture/web/` + `tools/g3-legacy-fixture/scripts/check_http_status.py` (G3 controlled fixture + HTTP-shape fail-closed helper, PRs #113–#116); (c) `tools/g2-candidate/` (G2 diagnostic shell per §3.3.2.1 row 1). These tools are available for diagnostic capture but **none of G2 Tier-2, G3 Tier-2, G4, G5, or G6 is promoted to PASS** by this reconciliation. G5 remains `blocked — baseline not reproducible; comparison not attempted` per §3.3.5 audit; G4 and G6 remain `blocked — verifiers not authored`. |
| **Non-goals (provisional scope)** | This reconciliation pass does **NOT**: (i) accept the final migrated product (no acceptance claim — only provisional infrastructure evidence); (ii) select Approach A / B / C or static export (Approach selection remains evidence-gated by G2 + G3 Tier-2 + G4 + G5 + G6); (iii) transfer FastAPI ownership or perform a cutover (no `WEB_DIR` repoint, no atomic cutover, no `api/server.py` / Makefile / extension / API / product-source change); (iv) promote G4 behavior parity to PASS (Playwright + Lighthouse harness remains blocked per §3.3.4); (v) promote G6 cutover rehearsal to PASS (`scripts/rehearse_cutover.py` remains not authored per §3.3.6). |
