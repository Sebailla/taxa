...
| G6 | `scripts/rehearse_cutover.py` referencing `design.md::§3.3.6` and the canonical `openspec/changes/migrate-nextjs-tailwind4/cutover-manifest.json` | **PR 1/5 scaffold authored (dry-run-only CLI, refuses without `--dry-run`, never executes `verification.command` / `rollback`, atomically writes a minimal `cutover-rehearsal.json`); full G6 validation still pending PRs 2–5** |
