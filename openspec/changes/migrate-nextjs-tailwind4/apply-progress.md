...
materially wrong against real `pytest -rs`:
                        ...
                        - **2026-09-13** — G6 slice 5 bounded canonical dry-run artifact closure evidence pass (this entry — **PR 5/6 documentation pass**; canonical `cutover-rehearsal.json` captured against the on-disk canonical `cutover-manifest.json`; **no source / tests / scripts / tasks / product files / evidence files / candidate workspace / `cutover-manifest.json` / `package-lock.json` are touched, committed, or pushed in this pass**). Per the parent task, the canonical G6 dry-run artifact is captured on this branch (`feat/g6-rehearsal-schema-validation-2`, commit `e1fc5a4`) and recorded verbatim here and in `design.md::§3.3.6` `Disposition (2026-09-13 — G6 slice 5 canonical dry-run artifact, bounded closure evidence)` row + status-footer update; the Spanish mirror `documents-es/.../apply-progress-es.md` carries the faithful Spanish translation. **G6 passes for canonical dry-run**; **G6 does NOT pass Tier-2 atomic-cut execution / rollback-rehearsal** (still evidence-gated by chosen Approach A / B / C + G4 PASS + G5 reproducible baseline); **G3 Tier-2, G4, G5 (steps 2 + 3), and cutover completion are NOT claimed in this pass.**
                          - **Canonical command + artifact** — `python3 scripts/rehearse_cutover.py --manifest openspec/changes/migrate-nextjs-tailwind4/cutover-manifest.json --out /tmp/g6-slice5-dry-run --dry-run` with `cwd = repo-root`; exit `0`; `--dry-run` is REQUIRED (no `--execute` variant is offered). Canonical artifact atomically emitted to `/tmp/g6-slice5-dry-run/cutover-rehearsal.json` (gitignored, reproducible on demand — same canonical manifest bytes → same artifact hash modulo wall-clock `validated_at`). Schema keys (exactly 10, frozen by slice-4 `EXPECTED_KEY_SET` pin): `consumer_count`, `consumer_ids`, `manifest_path`, `manifest_sha256`, `mode`, `out_dir`, `rollback_executed`, `validated_at`, `validation_errors`, `verification_executed`. NO `out_sha256`, NO `tier_2_passed`, NO `g3_tier2_passed`, NO `g4_passed`, NO `g5_passed`, NO `cutover_complete`, NO `rollback_rehearsed` keys exist (bounded-evidence pin — Tier-2 / cutover-completion keys are explicitly absent so the artifact cannot be misread as Tier-2 / cutover closure).
                          - **Canonical artifact facts** (verbatim, captured 2026-09-13): `manifest_path = "openspec/changes/migrate-nextjs-tailwind4/cutover-manifest.json"`; `manifest_sha256 = "3d228ebe5a98081ace5cf088858c0e7fe2b560c77815e5d68e5a01fb375e0c5d"` (matches `shasum -a 256` of the canonical manifest, stable across independent rehearsals); `validated_at = "2026-09-13T21:47:09.745252+00:00"` (ISO-8601 UTC); `mode = "dry-run"`; `consumer_count = 26`; `consumer_ids[]` = 26 unique IDs in canonical declaration order (21 `mount-*` §3.1.1 + 5 `search-urls-*` §3.1.2); `validation_errors = []`; `verification_executed = false`; `rollback_executed = false`.
                          - **Tests supporting the PASS** — `python3 -m pytest tests/test_rehearse_cutover.py tests/test_rehearse_cutover_path_text.py -q` → **149 passed in ~1.8s** (43 in `tests/test_rehearse_cutover.py` — 31 original PR 1/6–PR 4/6 tests + 10 slice-4 canonical artifact pins + 2 new slice-5 bounded-evidence pins: `test_canonical_artifact_bounded_evidence_no_tier2_or_cutover_markers` + `test_canonical_artifact_bounded_evidence_dry_run_only_no_execute_marker`; **106** in `tests/test_rehearse_cutover_path_text.py`), all green on this branch. No skips; no new failures.
                          - **Stability contract + focused tests** — PR #220 fail-closed branch collects all schema errors via `_validate_manifest`, logs every error to stderr, exits `EXIT_SCHEMA = 4`, emits no `cutover-rehearsal.json`; PR #218 `parse_shell_text` + `validate_repo_relative_path` helpers rule out absolute paths, `..` components, control chars / metachars, empty / null-shell, unmatched-quote `verification.command` / `rollback` strings before any further step. The artifact's `manifest_sha256` is stable across consecutive verifier runs (verified by slice-4 `test_canonical_artifact_manifest_sha256_stable_across_runs`). Two new focused tests (in PR 5/6 slice) pin the **bounded semantic intent** of the canonical artifact that the slice-4 key-set pin alone cannot catch (both target the canonical normalized manifest via the on-disk `_run_canonical` helper): `test_canonical_artifact_bounded_evidence_no_tier2_or_cutover_markers` (artifact does NOT carry any of `tier_2_passed`, `g3_tier2_passed`, `g4_passed`, `g5_passed`, `cutover_complete`, `rollback_rehearsed` keys) + `test_canonical_artifact_bounded_evidence_dry_run_only_no_execute_marker` (jointly `mode == "dry-run"` AND `verification_executed is False` AND `rollback_executed is False` — no `--execute` variant was offered).
                          **Truth preserved** — G6 **canonical dry-run PASS recorded 2026-09-13** (artifact emitted cleanly, 26 / 26 §3.1 consumer IDs, empty `validation_errors`, `verification_executed: false`, `rollback_executed: false`, `mode: "dry-run"`); **G6 Tier-2 atomic-cut execution / rollback-rehearsal NOT EXECUTED** (still evidence-gated by chosen Approach A / B / C + G4 PASS + G5 reproducible baseline); **G3 Tier-2 NOT PASSED** (still blocked by G4 + G5 + G6); **G4 NOT PASSED** (verifier not authored); **G5 closure-path step 1 complete** (per §3.3.5 `Disposition (2026-09-11 — G5 closure-path step 1)` row; steps 2 + 3 still blocked); **G6 disposition**: `canonical dry-run PASS recorded (bounded closure evidence); Tier-2 atomic-cut execution / rollback-rehearsal NOT YET EXECUTED`; **Approach A / B / C remain unselected**; **no FastAPI activation** (no `WEB_DIR` repoint, no atomic cutover, no `api/server.py` / Makefile / extension / API / product-source change); **canonical `cutover-manifest.json` unchanged**; the canonical artifact is the §3.3.6 gate evidence for canonical dry-run ONLY; it is **not** a Tier-2 / cutover-completion artifact. Spanish mirror updated in lockstep. No commit, push, or PR opened in this pass. Total authored additions stay well under the 400-line per-PR review budget.
