"""Seed the G3 legacy fixture SQLite DB with taxon + vernacular rows.

Idempotent: re-running deletes the file and rebuilds. Schema mirrors the
production v1+v2 layout (taxon + vernacular, no FTS triggers because the G3
verifier does not exercise search). Synthetic Panthera rows give every
vernacular a valid taxon_id (referential integrity check).

Run:
    python tools/g3-legacy-fixture/scripts/seed_db.py [path/to/taxa.db]
"""
...
# (id, taxon_id, name, language, country)
VERNACULAR_ROWS = [
    (1,  8, "Tiger",         "eng", "US"),
...
if __name__ == "__main__":
    out = Path(sys.argv[1] if len(sys.argv) > 1
               else "tools/g3-legacy-fixture/taxa.db")
    n_t, n_v = seed(out)
