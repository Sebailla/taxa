...
# ── Slice 3: bridge-advisory publication ───────────────────────────
...
def test_cli_docstring_header_includes_slice_3():
    """The CLI docstring header MUST mention Slice 3 (bridge-advisory
    publication) so reviewers can locate the bounded surface."""
    text = SCRIPT.read_text(encoding="utf-8")
    assert "Slices 1 + 1A + 2 + 3" in text, (
        "CLI docstring header must enumerate Slice 3 in addition to "
        "Slices 1 + 1A + 2 (it currently enumerates only 1 + 1A + 2).")
    ...
    def fake_bridge_factory(**kw):
        def b(url):
            return {
                ...
                "lhr": {"advisory": True},
                "provenance": {"advisory":
                    {"kind": "bridge_timeout", "reason": "x",
                     "timeout_s": 1.0}},
                "advisory": {"kind": "bridge_timeout", "reason": "x",
                              "timeout_s": 1.0},
            }
        return b

    monkeypatch.setattr(cli, "make_bridge", fake_bridge_factory)
    monkeypatch.setattr(cli.og, "run_orchestration",
                        lambda **kw: (kw["lifecycle"].start(),
                                          kw["lifecycle"].stop(),
                                          {"schema":
...
def test_cli_orchestrator_legacy_planner_ignores_kwarg_on_timeout(
    ...
    timeouts happened. This protects pre-slice-3 test fixtures."""
    sentinel_env = {
    ...
    monkeypatch.setattr(cli, "make_collector", fake_collector)
    monkeypatch.setattr(cli, "make_bridge",
                        lambda **kw: (lambda u: sentinel_env))
    captured: dict = {}
