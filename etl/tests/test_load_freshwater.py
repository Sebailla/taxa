...
def _seed_col_and_worms(conn: sqlite3.Connection) -> None:
    """Insert 2 CoL rows + 2 WoRMS-enriched rows for the idempotency test.

    The point is to assert these survive a wipe-and-reload run of the
    freshwater loader. CoL rows have coldp_id set; WoRMS-enriched rows have
    BOTH coldp_id and worms_id set. The freshwater loader must leave them
    alone (it only touches rows where freshwater_id IS NOT NULL).
    """
    conn.execute(
    ...
    conn.execute(
        "INSERT INTO taxon (parent_id, rank, status, scientific_name, "
        "authorship, coldp_id, worms_id, is_extinct) "
        "VALUES (2, 'kingdom', 'accepted', 'Chromista', NULL, NULL, 2, 0)"
    )
    conn.commit()


def test_load_freshwater_inserts_synthetic_root_and_orders(db_conn, write_csv, tmp_path):
...
def test_load_freshwater_skips_orphan_parents(db_conn, write_csv, tmp_path):
...
def test_load_freshwater_skips_empty_scientific_name(db_conn, write_csv, tmp_path):
...
def test_load_freshwater_skips_duplicate_freshwater_id(db_conn, write_csv, tmp_path):
...
def test_load_freshwater_is_idempotent(db_conn, write_csv, tmp_path):
...
def test_load_freshwater_rolls_up_species_count(db_conn, write_csv, tmp_path):
