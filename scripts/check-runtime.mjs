...
if (typeof SPEC !== "string" || !SPEC.trim()) {
  console.error(
    "[check-runtime] package.json::engines.node is missing or empty",
  );
  process.exit(1);
}
...
let cmp = 0;
for (let i = 0; i < 3; i++) {
  if (observed[i] !== required[i]) {
    cmp = observed[i] > required[i] ? 1 : -1;
    break;
  }
}
if (cmp < 0) {
  console.error(
    `[check-runtime] Node ${process.versions.node} is below the required ${FLOOR} floor (package.json::engines.node = ${JSON.stringify(SPEC)}). Upgrade Node per .nvmrc.`,
  );
  process.exit(1);
}
console.log(
  `[check-runtime] Node ${process.versions.node} >= ${FLOOR} OK (engines.node = ${JSON.stringify(SPEC)})`,
);
process.exit(0);
