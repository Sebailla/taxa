...
import argparse, contextlib, datetime as _dt, hashlib, json, os, platform, re, shutil, sys, uuid
...
class PlaywrightBrowserAdapter:
    ...
    LIFECYCLE (the seam this slice exists to protect):
    `playwright.sync_api.sync_playwright()` returns a
    `_PlaywrightContextManager`. The Playwright instance that carries
    `.chromium` only exists inside the `with sync_playwright() as p:`
    block. Every chromium access below therefore goes through
    `_pw_ctx()`, which yields the active Playwright instance only
    while the context manager is entered. Touching `.chromium`
    outside that block raises AttributeError on the real API."""

    def __init__(self, *, headless: bool = True) -> None:
        self._headless: bool = headless

    @contextlib.contextmanager
    def _pw_ctx(self) -> Iterator[Any]:
        """Yield a Playwright instance inside an active
        `sync_playwright()` context. `.chromium` is only valid
        inside this block — accessing it outside raises an
        AttributeError on the real API and a RuntimeError on our
        strict context-manager fake."""
        try:
    ...
    def chromium_provenance(self) -> dict:
        with self._pw_ctx() as p:
            b = p.chromium.launch(headless=self._headless)
            try:
                cv, ce = (b.version or "unknown"), p.chromium.executable_path
            finally:
                b.close()
        return {"version": cv, "executable_path": ce}

    def playwright_provenance(self) -> dict:
        # Adaptation: source `80b711c` accessed `playwright.__version__`;
        # Playwright 1.62.0 (pinned in requirements-dev.txt) drops that
        # attribute. Fall back through `importlib.metadata.version` so the
        # lazy "missing-install → unknown" contract still holds.
        try:
        ...
        try:
            ...
            return {"version": _md.version("playwright")}
        except Exception:
